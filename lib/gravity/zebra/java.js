        	...
        	$(function(container) {
        	    if (isBA && container instanceof ArrayBuffer) {    
            	    ...
            	    if (zebra.isString(container)) {
            	        ...
            	        if (container.constructor != Array) throw new IllegalArgumentException("Wrong type: " + typeof(container));
            	    }
            	    this.data = container; 
        	    }
        	    this.marked  = -1; 
...
//!!!                        if (!c.equals(this.color)) {
                            this.target.fillStyle   = c.s;
                            this.target.strokeStyle = this.target.fillStyle;
                            this.color = c;
  //                      }
                    }
                ...
                $(function drawXORRect(x,y,w,h){
                    try {
                        this.setXORMode();
                        this.setColor(Color.black);
                        this.drawRect(x, y, w, h);
                    }
                    finally { this.setPaintMode(); }
                });

                $(function fillXORRect(x,y,w,h){
                    try {
                        this.setXORMode();
                ...
                $(function drawImage(img,dx1,dy1,dx2,dy2,sx1,sy1,sx2,sy2){
                    var ctx = this.target;
                    ctx.drawImage(img, sx1, sy1, sx2 - sx1, sy2 - sy1, dx1, dy1, dx2 - dx1, dy2 - dy1); 
                });
                ...
                $(function setXORMode() { this.target.globalCompositeOperation = "xor"; });
                $(function setPaintMode(){ this.target.globalCompositeOperation = "source-over"; });
        ...
        pkg.Font = Class(function($) {
            ...
            function CanvasMetrics(canvas) {
                if (zebra.isString(canvas)) canvas = (typeof(document) !== "undefined" ? document.getElementById(canvas) : null);
                if (canvas == null) {
                    this.measureText = function(f, t) { return 0; } 
                    this.getAscent   = function(f) { return 0; }
                    this.getHeight   = function(f) { return 0; }
                    return;
                }

                var ctx = canvas.getContext("2d");
                this.measureText = function(f, t) { 
                    ctx.font = f.s; 
                    return Math.round(ctx.measureText(t).width);
                    //!!!return l2 + ((l1-l2)>0?1:0);
                } 

            	var fmtxt = document.getElementById("JAVA.awt.fm.text"), fmimg = document.getElementById("JAVA.awt.fm.image");
                this.getAscent = function(f) {
                    fmtxt.style.font = f.s;
                    return fmimg.offsetTop - fmtxt.offsetTop + 1;
                }

                this.getHeight = function(f) {
                    fmtxt.style.font = f.s;
                    return fmtxt.offsetHeight;
                }
            }
            ...
            function metrics()  {
                if (METRICS == null) METRICS = new CanvasMetrics("JAVA.awt.fm.canvas"); 
                return METRICS;
            }
        	...
        	$(function (name, style, size) {
        	    ...
        	    this.style  = style;
        	    this.size   = size;
        	    this.s      = [(this.style & Font.ITALIC)>0?'italic ':'', (style & Font.BOLD) > 0? 'bold ':'', this.size, 'px ', this.name].join('');
        	
        	    this.height = this.ascent = -1;

            	this.getAscent = function() { 
            	    if (this.ascent == -1) this.ascent = metrics().getAscent(this);
            	    return this.ascent; 
            	}

            	this.getHeight = function() { 
            	    if (this.height == -1) this.height = metrics().getHeight(this);
            	    return this.height; 
            	}

                this.stringWidth = function(s) { return metrics().measureText(this, s); }
                this.charsWidth = function(s, off, len) { 
                    return len == 1 ? metrics().measureText(this, s[off]) : metrics().measureText(this, s.substring(off, off + len)); 
                    //return metrics().measureText(this, s.substring(off, off + len)); 
                }
        	});
