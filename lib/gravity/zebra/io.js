(function(pkg, Class, Interface) {
    ...
    pkg.InputStream = Class([
    	...
    	function mark(readlimit) { throw new Error(); },
    	function reset() { throw new Error(); },
    	function close() {},
    	...
    	function readByte() { throw new Error(); }
    ]);

    var isBA = typeof(ArrayBuffer) !== 'undefined';
    pkg.ByteArrayInputStream = Class(pkg.InputStream, [
    	function(container) {
    ...
    pkg.FileInputStream = Class(pkg.ByteArrayInputStream, function($) {
        function download(path, target) {
            if(zebra.isIE) {
                ...
                if (data.length <= 0) throw new Error(path); 
                return data;
            }
            else {
...
//                    if (b) req.responseType = 'arraybuffer';
    ...
    pkg.Reader = Class(function($) {
        ...
        $(function markSupported() { return false; });
        $(function skip(n) { throw new Error(); });
        $(function mark(n) { throw new Error(); });
        $(function mark() { this.mark(0); });
        $(function reset() { throw new Error(); });
...
})(zebra("io"), zebra.Class, zebra.Interface);
