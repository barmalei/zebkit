...
var Cursor = JAVA.awt.Cursor, TextModel = zebra.data.TextModel, Render = pkg.view.Render, temporary = new Rectangle();
...
pkg.$objects = new Bag([
    function() { 
        ...
        var $this = this;
        this._.add(function(key, id, obj) {
            if (obj != null && zebra.instanceOf(obj, pkg.view.ImgRender)) $this.images.push(obj.target);
        });
    },
    
    function load(is) {
        this.$super(is);
        if (timer.get(this) == null && this.images.length > 0) { 
            zebra.busy();
            timer.run(this, 150, 80);
        }
    },
    ...
    function run() {
        ...
        if (l > 0) {
            for(var i=0; i < l; i++)  {
                ...
                if (img.complete != true) this.images.push(img);
            }
        }
        
        if (this.images.length == 0 && timer.get(this) != null) { 
            timer.remove(this);
            zebra.ready();
        }
    },
...
pkg.load = function(path) { pkg.$objects.load((new JAVA.io.TextFileReader(path)).text); }
                    ...
                    for(var i = 0;i < count; i++){
                        ...
                        if(kid.isVisible){
                            ...
                            if(iw > 0 && ih > 0) {
                                ...
                                // can.width = kid.width;
                                // can.height = kid.height;
                                // var gg = new JAVA.awt.Graphics(can);
                                ...
                                //!!!
                                //g.target.drawImage(gg.target, kidX, kidY);
...
zebra.ready(function() {    
    var p = zebra.env["ui.theme"]
    pkg.load(p ? zebra.env["ui.theme"] : "p3d/gray.theme.properties");
});
