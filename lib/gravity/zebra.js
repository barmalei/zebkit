...
pkg.$cache = function(name, clazz) {
    if (($cached[name] && $cached[name] != clazz) || pkg.$global[name]) throw Error("Class name conflict: " + name);
    $cached[name] = clazz;
}

pkg.setupNames = function(pkg, doCache) {
    for(var k in pkg) {
        var c = pkg[k];
        if (c && zebra.instanceOf(c, Class)) { 
            c.$name = k;
            if (c && doCache === true) zebra.$cache(k, c);
        }
    }
}
...
Class.forName = function(name) { 
    ...
    //!!!!!! infinite cache !!!!
    if ($cached.hasOwnProperty(name) == false) zebra.$cache(name, eval(name));
    return $cached[name];
}
