...
//!!!!
        ...
        this.get = function(i){
            if (i < 0 || i >= this.kids.length) throw new Error("Index  " + i + " is out of range");
            return this.kids[i];
        };
        ...
        this.doLayout = function (lw){
            ...
            for(var i = 0;i < lw.kids.length; i++){
                ...
                if(cc.isVisible){
                    ...
                    cc.setLocation((constr == pkg.STRETCH) ? x   : x + pkg.getXLoc(cc.width, constr, psw), y);
                    y += (d.height + this.gap);
                }
            }
        };
    ...
    try {
        ...
        if (zebra.isInBrowser && pkg.useWebClipboard) {
            ...
            $clipboard.onkeyup = function(ee) {
                zebra.print("onkeyup : " + ee.keyCode);
            ...
            if (zebra.isFF) {
                $clipboard.addEventListener ("input", function(ee) {
                    if (pkg.focusManager.focusOwner && pkg.focusManager.focusOwner.paste) {
                        zebra.print("input " + $clipboard.value);
                ...
                $clipboard.onpaste = function(ee) {
                    zebra.print("::: onpaste() " + $clipboard.value + "," + ee.clipboardData);
                    if (pkg.focusManager.focusOwner && pkg.focusManager.focusOwner.paste) {
                        var txt = ee.clipboardData ? ee.clipboardData.getData('text/plain')
...
pkg.Menu = Class(pkg.CompList, pkg.ChildrenListener, [
    ...
    function posChanged(target,prevOffset,prevLine,prevCol){
        ...
        if (off < 0 || (this.kids.length > 0 && this.kids[off].isVisible)){
            ...
            for(; cc > 0 && (this.get(off).isVisible === false || this.isDecorative(off)); cc--){
                off += d;
                if (off < 0) off = ccc - 1;
                if (off >= ccc) off = 0;
            }
    ...
    function $prototype() {
        ...
        this.posChanged = function (target, prevOffset, prevLine, prevCol){
            ...
            if(selectedIndex >= 0){
                ...
                if (index + 1 < this.kids.length && sub != this.get(index + 1)) {
                    this.removeAt(index + 1);
                }

                if (index + 1 == this.kids.length && sub != null) {
                    timer.run(this, 900, 5000);
                }
            }
        };
