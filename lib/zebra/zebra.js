...
pkg.Bag = zebra.Class([
    function $prototype() {
         ...
         * @return {zebra.util.Bag} the bag itself
         * @method load
         */
 ...
 * @param  {zebra.layout.Layoutable} c a component
 * @return {Array} an absolute location of the given location of the specified 
...
pkg.Font = function(name, style, size) {
    ...
    //!!!
    // Something weird is going on in IE10 !
            ...
            if (w > 0 && h > 0 && c.isVisible === true){
                ...
                if(canvas != null){
                    ...
                    if (w > 0 && h > 0)
                    {
                        ...
                        if (da.width > 0) {
                            if (x >= da.x && y >= da.y && 
                                x + w <= da.x + da.width && y + h <= da.y + da.height)
                            { 
                                return;
                            }
...
pkg.PaintManImpl = Class(pkg.PaintManager, CL, [
    function $prototype() {
        ...
        this.paintComponent = function(g,c){
            ...
            if (c.paint) {
                var left = c.getLeft(), top = c.getTop(), 
                    bottom = c.getBottom(), right = c.getRight(), 
                    id = -1;
...
pkg.CursorManager = Class(pkg.Manager, MouseListener, [
    function $prototype() {
         ...
         * @param  {zebra.ui.Cursorable|String} c a zebra.ui.Cursorable 
    ...
    function(){
        this.$super();
        /**
         * Keeps cursor info class instances for the number of UI components
         * @attribute cursors
         * @private
         * @readOnly
         * @type {Object}
...
pkg.Checkbox = Class(pkg.StatePan, Actionable, [
    ...
    function (c, m){
        ...
        else           this.add(new pkg.Checkbox.Box());
...
pkg.Extender = Class(pkg.Panel, MouseListener, Composite, [
    function $prototype() {
        this.catchInput = function(child){ 
            return child != this.contentPan && 
                   L.isAncestorOf(this.contentPan, child) === false; 
        };
        ...
        this.mousePressed = function(e){
            if (e.isActionMask() && this.getComponentAt(e.x, e.y) == this.togglePan) {
                this.toggle();
            }
        };
    },
...
pkg.WinLayer = Class(pkg.BaseLayer, pkg.ChildrenListener, [
    function $clazz() {
        ...
        this.activate = function(c) {
            pkg.findCanvas(c).getLayer("win").activate(c);
        };
    },
        ...
        this.posChanged = function (target, prevOffset, prevLine, prevCol){
            ...
            if (selectedIndex >= 0){
                var index = this.pcMap.indexOf(target), 
                    sub = this.kids[index].getSubmenuAt(selectedIndex);
...
var L = zebra.layout, Cursor = ui.Cursor, KeyEvent = ui.KeyEvent,
    Cursorable = ui.Cursorable, CURSORS = [];
