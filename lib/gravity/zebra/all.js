...
pkg.zCanvas = Class(pkg.Panel, pkg.Desktop, function($) {
    ...
    $(function(canvas, w, h) {
        ...
        if (e.offsetParent) {
        do {
                this.offx += parseInt(e.offsetLeft, 10);
                this.offy += parseInt(e.offsetTop, 10);
...
pkg.Tree = Class(ui.Panel, ui.MouseListener, ui.KeyListener,
    ...
    function paintBranch(t, g,root){
        ...
        if(t.visibleArea.intersects(node.x + dx, node.y + dy, node.width, node.height)){
            ...
            if(toggleView != null) toggleView.paint(g, toggle.x, toggle.y, toggle.width, toggle.height, t);
...
var ScrollListener = ui.ScrollListener, TextRender = ui.view.TextRender, View = ui.view.View,
    ...
    Position = zebra.util.Position, Rectangle = zebra.util.Rectangle,
...
pkg.Grid = Class(ui.Panel, ui.MouseListener, ui.KeyListener, Position.PositionMetric,
        ...
        $(function paintData(g){
            var y = this.visibility.fr[1] + this.cellInsetsTop, addW = this.cellInsetsLeft + this.cellInsetsRight, 
                addH = this.cellInsetsTop + this.cellInsetsBottom, ts = g.getTopStack(), cx = ts.x, cy = ts.y, cw = ts.width, ch = ts.height, res = new Rectangle();
