...
pkg.Class = make_template(null, function() {
    ...
    $template.Field = function(n, v, m) {
        ...
        if (typeof v === 'function') {
            ...
            if (m == ABSTRACT) { 
                ...
                v = function() { throw new Error("Abstract '" + n + "(" + arity +")" + "' cannot be called"); }; 
            }
    ...
    if (typeof df === 'function') df.call($template, function(n, v) { $template.Field(n, v); } );
    else {
...
pkg.Layoutable = Class(L, [
    ...
    function setLayout(m){
        if (!m) throw new Error("Undefined layout"); 
...
var Dimension = zebra.util.Dimension, rgb = zebra.util.rgb, T = zebra.ui, L = zebra.layout, PositionMetric = zebra.util.Position.PositionMetric;
...
pkg.TextRender = Class(pkg.Render, PositionMetric, [
    ...
    function textUpdated(src,b,off,size,ful,updatedLines){
        if (b == false) {
            if(this.lines > 0){
    ...
    function getPreferredSize(){
        if(this.width < 0){
            ...
            if(this.views.length > 0){
                for(var i = 0;i < ~~(this.views.length / 2); i++){
...
pkg.InputEventPan = Class(pkg.Panel, MouseListener,  MouseMotionListener, KeyListener, function($) {
    ...
    $(function keyReleased(e){
        if(this.state == PRESSED_OVER || this.state == PRESSED_OUT){
            ...
            if (this.isMCInside == false) this.updateState(OUT);
        }
    });
...
pkg.ImagePan = Class(pkg.Panel, [
    ...
    function (v){
        if (v) {
            ...
            if (v.constructor === Image)  v = new pkg.view.Picture(v);
        } 
        this.$super(); 
        this.setView(v);
        this.setBackground(null);
    ...
    function fd(t,index,d){
        if(t.kids.length > 0){
            ...
            for(var i = index;i >= 0 && i < t.kids.length; i += d){
                var cc = t.kids[i];
                if(cc.isEnabled && cc.isVisible && cc.width > 0 && cc.height > 0 && (isNComposite || (t.catchInput && t.catchInput(cc) == false)) && (cc.canHaveFocus() || (cc = fd(cc, d > 0 ? 0 : cc.kids.length - 1, d)) != null))
                    return cc;
            }
        }
        return null;
...
pkg.ScrollPan = Class(pkg.Panel, pkg.ScrollListener, function($) {
    ...
    var ContentPan = Class(pkg.Panel,[
            function (c){
                ...
                this.sman = new pkg.SimpleScrollManager(c, function($) {
    ...
    $(function getItemIdxAt(x,y){
        ...
        if(this.vArea != null && this.firstVisible >= 0){
            var yy = this.firstVisibleY + this.getScrollManager().getSY(), hh = this.height - this.getBottom(),
                count = this.model.elementsCount(), gap = this.getItemGaps() * 2;
            for(var i = this.firstVisible;i < count; i++) {
...
pkg.WinLayer = Class(pkg.BaseLayer, pkg.ChildrenListener, function($) {
    ...
    $(function addWin(win, listener) { this.addWin(WL.MDI, win, listener); });
...
pkg.Window = Class(pkg.Panel, pkg.MouseMotionListener, WinListener,
    ...
    this.show = function(canvas, win, modal) {
        canvas.getLayer(pkg.WinLayer.ID).addWin(modal === true ? WinLayer.MODAL : WinLayer.MDI, win);
    }
        ...
        this.run = function(){
            if(this.tooltip == null){
                ...
                if(this.tooltip != null) {
                    ...
                    this.targetLayer.addWin(pkg.WinLayer.INFO, this.tooltip);
                }
            }
        }
    });
...
var ScrollListener = ui.ScrollListener, TextRender = ui.view.TextRender, View = ui.view.View,
    ...
    Position = zebra.util.Position, Rectangle = zebra.util.Rectangle, Point = zebra.util.Point,
    KE = ui.KeyEvent, rgb = zebra.util.rgb, Dimension = zebra.util.Dimension;
    ...
    $(function paint(g){
        if(this.metrics != null){
            ...
            var x = isHor ? cv.fc.y - this.x + m.getXOrigin() - gap : this.getLeft(),
                y = isHor ? this.getTop() : cv.fr.y - this.y + m.getYOrigin() - gap,
        ...
        if(this.metrics != null && x >= 0 && y >= 0 && x < this.width && y < this.height){
            ...
            if((isHor && cv.fc != null) || ( ! isHor && cv.fr != null)){
                var gap = m.lineSize, xy = isHor ? x : y,
                    xxyy = isHor ? cv.fc.y - this.x - gap + m.getXOrigin() : cv.fr.y - this.y - gap + m.getYOrigin();
                for(var i = (isHor ? cv.fc.x : cv.fr.x);i <= (isHor ? cv.lc.x : cv.lr.x); i ++ ){
    ...
    function calcRowColAt(t, x,y){
        ...
        if((isHor && cv.fc != null) || (!isHor && cv.fr != null)){
            var m = t.metrics, g = m.lineSize, xy = isHor ? x : y,
                xxyy = isHor ? cv.fc.y - t.x + m.getXOrigin() - g : cv.fr.y - t.y + m.getYOrigin() - g;
            for(var i = (isHor ? cv.fc.x : cv.fr.x);i <= (isHor ? cv.lc.x : cv.lr.x); i ++ ){
                var wh = isHor ? m.getColWidth(i) : m.getRowHeight(i);
                xxyy += (wh + g);
...
pkg.Grid = Class(ui.Panel, ui.MouseListener, ui.KeyListener, Position.PositionMetric,
        ...
        $(function paintNet(g){
            var v = this.visibility, topX = v.fc.y - this.lineSize, topY = v.fr.y - this.lineSize,
                botX = v.lc.y + this.colWidths[v.lc.x], botY = v.lr.y + this.rowHeights[v.lr.x];
        ...
        $(function cellByLocation(x,y){
            ...
            var dx = this.sman.getSX(), dy = this.sman.getSY(), v = this.visibility,
                ry1 = v.fr.y + dy, rx1 = v.fc.y + dx, row =  -1, col =  -1,
                ry2 = v.lr.y + this.rowHeights[v.lr.x] + dy,
                rx2 = v.lc.y + this.colWidths[v.lc.x] + dx;
            if(y > ry1 && y < ry2){
...
var RasterLayout = zebra.layout.RasterLayout,
    L = zebra.layout, Rectangle = zebra.util.Rectangle, Layer = zebra.ui.Layer,
    Point = zebra.util.Point, Cursor = ui.Cursor, Dimension = zebra.util.Dimension,
    Panel = zebra.ui.Panel, KeyListener = zebra.ui.KeyListener, MouseMotionListener = zebra.ui.MouseMotionListener,
