...
var ExternalEditor = ui.ExternalEditor, WinLayer = ui.WinLayer, MB = zebra.util, Cursor = JAVA.awt.Cursor;
var Position = zebra.util.Position, Rectangle = JAVA.awt.Rectangle;
var KE = ui.KeyEvent, Color = JAVA.awt.Color, Dimension = JAVA.awt.Dimension, Point = JAVA.awt.Point;
...
function CellsVisibility() { 
    this.getFirstCell = function(){
        return this.hasVisibleCells() ? new Point(this.fr.x, this.fc.x) : null;
    }

    this.getLastCell = function(){
        return this.hasVisibleCells() ? new Point(this.lr.x, this.lc.x) : null;
    }

    this.getFirstCellLoc = function(){
        return this.hasVisibleCells() ? new Point(this.fc.y, this.fr.y) : null;
    }

    this.getLastCellLoc = function(){
        return this.hasVisibleCells() ? new Point(this.lc.y, this.lr.y) : null;
    }

    this.cancelVisibleCells = function(){
        this.fr = null;
    }
...
pkg.DefViews = Class([
    ...
    function getRender(){ return this.defView; }
]);
...
pkg.GridCaption = Class(ui.Panel, ui.MouseMotionListener, ui.MouseListener, ui.Cursorable, function($) {
        ...
        $(function (m,o){
            ...
            this.borderView = this.render = null;
...
pkg.Grid = Class(ui.Panel, zebra.data.MatrixModelListener, ui.MouseListener, ui.KeyListener, Position.PositionMetric,
        ...
        $(function paint(g){
            ...
            if(this.visibility.hasVisibleCells()){
                var sx = this.sman.getSX(), sy = this.sman.getSY(), cw = 0;
                try{
                    g.translate(sx, sy);
                    this.paintSelection(g);
                    if(this.leftCaption != null || this.topCaption != null){
                        cw = g.clip.width;
                        var cx = g.clip.x, cy = g.clip.y, ch = g.clip.height, dx = this.sman.getSX(), dy = this.sman.getSY();
        ...
        $(function setRowHeight(row,h){
            ...
            if( !this.isUsePsMetric){
                ...
                if(this.rowHeights[row] != h){
                    ...
                    this.updateCashedPs(-1, this.getTop() + this.getBottom() + this.psHeight_ + ((this.topCaption != null && this.topCaption.isVisible) ? this.topCaption.getPreferredSize().height : 0));
        ...
        $(function setColWidth(col,w){
            if(w < 0) throw new IllegalArgumentException("" + w);
            ...
            if( !this.isUsePsMetric){
                ...
                if(this.colWidths[col] != w){
                    ...
                    this.updateCashedPs(this.getRight() + this.getLeft() + this.psWidth_ + ((this.leftCaption != null && this.leftCaption.isVisible) ? this.leftCaption.getPreferredSize().width : 0),  - 1);
                    if(this.parent != null) this.parent.invalidate();
                    this.iColVisibility(0);
                    this.invalidateLayout();
                    this.repaint();
        ...
        $(function paintData(g){
            ...
            var cx = g.clip.x, cy = g.clip.y, cw = g.clip.width, ch = g.clip.height, res = new Rectangle();
                    ...
                    for(var j = this.visibility.fc.x;j <= this.visibility.lc.x; j ++ ){
                        ...
                        if(v != null){
                            ...
                            if(res.width > 0 && res.height > 0){
                                g.setClip(res.x, res.y, res.width, res.height);
