...
var WinListener = pkg.WinListener, Actionable = zebra.util.Actionable, ComponentListener = pkg.ComponentListener;
...
pkg.WinLayer = Class(pkg.BaseLayer, pkg.ChildrenListener, function($) {
    ...
    $(function add(c,l,type){ this.add([type, l], c); });
    ...
    $(function kidAdded(index,id,lw){
        ...
        if(info[1] != WL.MDI && info[1] != WL.MODAL && info[1] != WL.INFO) throw new IllegalArgumentException("100");
    ...
    $(function activate(c){
        if(c != null && (this.winsInfo.hasOwnProperty(c) == false || winType(this, c) == WL.INFO)) throw new IllegalArgumentException("101");
        if(c != this.activeWin){
            if(c == null){
                if (winType(this, this.activeWin) == WL.MODAL) throw new IllegalArgumentException("102");
                ...
                if(this.winsStack.indexOf(c) < this.topModalIndex) throw new IllegalArgumentException("103");
                var old = this.activeWin;
                this.activeWin = c;
...
pkg.WinProducer = Class(pkg.Switchable, function($) {
    var WinProducer = this;
    
    this.ICON_EL = 1;
    this.CLOSE_BUTTON_EL = 2;
    this.SIZING_EL = 3;
    
    $(function (){
        this.targetWin = null;
        this.root = this.createRoot();
        this.captionComponent = this.createCaption();
        this.statusComponent = this.createStatus();
    });

    $(function setTargetWin(win){ this.targetWin = win; });
    $(function getTargetWin(){ return this.targetWin; });
    $(function getCaptionComp(){ return this.captionComponent; });
    $(function getRoot(){ return this.root;});
    $(function getStatusComp(){ return this.statusComponent; });
    $(function getElement(type){ return this.fetchElement(type); });

    $(function setElement(type,c){
        var old = this.getElement(type);
        if(old != c) {
            if(zebra.instanceOf(old, Actionable)) old._.remove(this);
            if(zebra.instanceOf(c, Actionable)) c._.add(this);
            if(old != null){
                var parent = old.parent, index = parent.indexOf(old);
                if(index >= 0) this.parent.removeAt(index);
            }
            if(c != null && c.parent == null) this.addElement(type, c);
            if(this.targetWin != null) this.targetWin.repaint();
        }
    });

    $(function fired(src){
        var l = this.targetWin.parent;
        if(l != null && src == this.getElement(WinProducer.CLOSE_BUTTON_EL)) l.remove(this.targetWin);
    });

    $(function getElementTypes(){
        return [WinProducer.ICON_EL, WinProducer.CLOSE_BUTTON_EL, WinProducer.SIZING_EL];
    });

    $(function createStatus(){
        var status = new pkg.Panel(new L.FlowLayout(L.RIGHT, L.CENTER));
        status.add(new pkg.ImagePan(pkg.get("win.corner")));
        status.setBackground(null);
        return status;
    });

    $(function createRoot(){ return new pkg.Panel(); });
    $(function createCaption(){ return null; });
    $(function addElement(type,el){ if(type == WinProducer.SIZING_EL) this.getStatusComp().add(el); });

    $(function fetchElement(type){
        return type == WinProducer.SIZING_EL ? this.statusComponent != null && 
               this.statusComponent.kids.length > 0 ? this.statusComponent.kids[0] : null : null;
    });

    $(function switched(b){});
    
    this.Abstract(function getTitle(){ });
    this.Abstract(function setTitle(s){ });
});

pkg.BWinProducer = Class(pkg.WinProducer, function($) {
    var WP = pkg.WinProducer;
   
    $(function() {
        this.buttons = this.icons = this.title = null;
        this.activeBgView = pkg.get("win.baseprod.cap.av");
        this.inactiveBgView = pkg.get("win.baseprod.cap.iv");
        this.$super();
        if(this.getCaptionComp() != null) this.getCaptionComp().setBackground(this.inactiveBgView);
    });
    
    $(function setTitle(s){ this.title.setText(s); });
    $(function getTitle(){ return this.title.getText(); });

    $(function switched(state){
        if(this.getCaptionComp() != null){
            this.getCaptionComp().setBackground(state ? this.activeBgView : this.inactiveBgView, true);
        }
    });

    $(function setCaptionColor(state,c){
        var old = state ? this.activeBgView : this.inactiveBgView;
        if( !old.equals(c)){
            var title = this.getCaptionComp(), isActive = (title.bg == this.activeBgView);
            if (state) this.activeBgView = c;
            else this.inactiveBgView = c;
            if (title != null) title.setBackground(isActive ? this.activeBgView : this.inactiveBgView);
        }
    });

    $(function createCaption(){
        var titlePanel = new pkg.Panel(new L.BorderLayout(2, 2));
        titlePanel.padding(1);
        ...
        this.title.setForeground(JAVA.awt.Color.white);
        ...
        var ic = new pkg.ImagePan(pkg.get("win.icon"));
        this.setElement(WP.ICON_EL, ic);
        this.icons.add(this.title);
        titlePanel.add(L.LEFT, this.icons);
        titlePanel.add(L.RIGHT, this.buttons);
        this.setElement(WP.CLOSE_BUTTON_EL, exit);
        return titlePanel;
    });

    $(function fetchElement(type){
        if(type == WP.CLOSE_BUTTON_EL) return this.buttons.kids.length > 0 ? this.buttons.kids[0] : null;
        return (type == WP.ICON_EL) ? (this.icons.kids.length > 1 ? this.icons.kids[0] : null) : this.$super(this.fetchElement,type);
    });

    $(function addElement(type,el){
        if(type == WP.ICON_EL) this.icons.insert(0, null, el);
        else if(type == WP.CLOSE_BUTTON_EL) this.buttons.add(el);
        this.$super(this.addElement,type, el);
    });
});
...
pkg.Window = Class(pkg.Panel, pkg.MouseMotionListener, pkg.WinListener,
    ...
    $(function (p,s){
        ...
        this.$super();
        this.setWinProducer(p);
        this.master.setTitle(s);
    ...
    $(function setWinProducer(m){
        if(m != this.master){
            if(this.master != null){
                this.master.setTargetWin(null);
                this.removeAll();
            }
            this.master = m;
            if(this.master != null){
                this.master.setTargetWin(this);
                this.add(L.CENTER, this.master.getRoot());
                if(this.master.getCaptionComp() != null) this.add(L.TOP, this.master.getCaptionComp());
                if(this.master.getStatusComp() != null) this.add(L.BOTTOM, this.master.getStatusComp());
            }
        }
    });
    ...
    $(function getWinProducer(){ return this.master; });

    $(function setSizeable(b){
        if(this.isSizeable != b){
            this.isSizeable = b;
            if(this.getWinProducer().getElement(pkg.WinProducer.SIZING_EL) != null){
                this.getWinProducer().getElement(pkg.WinProducer.SIZING_EL).setVisible(b);
            }
        }
    });

    $(function getRoot(){ return this.master.getRoot(); });
    ...
    $(function winOpened(winLayer,target,b){ if(this.master != null) this.master.switched(false); });
    $(function winActivated(winLayer,target,b){ if(this.master != null) this.master.switched(b); });

    $(function getCursorType(target,x,y){
        return (this.isSizeable && insideCorner(this, x, y)) ? JAVA.awt.Cursor.SE_RESIZE : -1;
    });

    $(function catchInput(c){
        var tp = this.master.getCaptionComp();
        return c == tp || (L.isAncestorOf(tp, c) && !(zebra.instanceOf(c, pkg.Button))) || this.master.getElement(pkg.WinProducer.SIZING_EL) == c;
    });

    $(function mouseClicked(e){
        var x = e.x, y = e.y, cc = this.master.getCaptionComp();
    ...
    function focusStateUpdated(t){
        var c = t.master.getCaptionComp();
        if(c != null) c.repaint();
    }

    function insideCorner(t, px,py){ return t.getComponentAt(px, py) == t.master.getElement(pkg.WinProducer.SIZING_EL); }
});

pkg.TooltipManager = Class(pkg.MouseListener, pkg.MouseMotionListener, function($) {
...
pkg.Menu = Class(pkg.CompList, pkg.ChildrenListener, function($) {
    ...
    $(function insert(i,o,c){ this.$super(this.insert,i, o, this.createItem(c));});
    ...
    $(function setSubMenu(index,menu){
        if(index >= this.kids.length || menu == this || isDecorative(this, index)) throw new IllegalArgumentException("2002");
    ...
    $(function layout(target){
        ...
        if(hasSub){
            ...
            l.setLocation(left + ew - ps.width, top + Math.round((eh - ps.height) / 2));
            l.setSize(ps.width, ps.height);
        }
        if(count > 2 || (!hasSub && count > 1)){
            l = target.kids[0];
            var ps = l.getPreferredSize();
            l.setLocation(x, top + Math.round((eh - ps.height) / 2));
            l.setSize(ps.width, ps.height);
            l = target.kids[1];
        }
        else l = target.kids[0];
        var ps = l.getPreferredSize();
        l.setLocation(x + this.iconAreaWidth, top + Math.round((eh - ps.height) / 2));
    ...
    function isDecorative(t, index){
        return !(zebra.instanceOf(t.kids[index], pkg.Panel) && zebra.instanceOf(t.kids[index].getLayout(), pkg.Menu));
    }
});
...
pkg.MenuBar = Class(pkg.Panel, pkg.ChildrenListener, pkg.KeyListener, pkg.Switchable, function($) {
    ...
    $(function getView(type){
        if(type != MenuBar.ON_BR_VIEW && type != MenuBar.OFF_BR_VIEW) throw new IllegalArgumentException("2003");
...
pkg.PopupLayer = Class(pkg.BaseLayer, pkg.ChildrenListener, function($) {
    ...
    $(function calcPreferredSize(target){ return new JAVA.awt.Dimension(); });
...
pkg.PopupManager = Class(pkg.MouseListener, [
    function () { 
        this.menus = {};
...
pkg.Line = Class(pkg.Panel, [
    ...
    function (b, orient){
        this.$super();
        if(orient != L.HORIZONTAL && orient != L.VERTICAL) throw new IllegalArgumentException("2001");
        //!!!! 
        // b is not used
    ...
    function paint(g){
        ...
        if(this.orient == L.HORIZONTAL) {
            var yy = this.top + Math.floor((this.height - this.top - this.bottom - 1) / 2);
            g.drawLine(this.left, 3, this.width - this.right - this.left, 3);
        }
        else {
            var xx = this.left + Math.floor((this.width - this.left - this.right - 1) / 2);
            g.drawLine(xx, this.top, xx, this.height - this.top - this.bottom);
        }
    },
    
    function getPreferredSize() { return new JAVA.awt.Dimension(5,5); }
]);
