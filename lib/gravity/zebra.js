...
var ABSTRACT = 1, FINAL = 2, namespaces = {}, namespace = function(nsname, dontCreate) {
    if (!isString(nsname)) throw new Error("Wrong nsname argument");
...
pkg.IllegalArgumentException = function(msg) { Error.call(this, msg); }
pkg.IllegalArgumentException.prototype = new Error();
...
$Fields.prototype.addField = function(n, v, owner, m) {   
...
pkg.Class = make_template(null, function() {
    ...
    $template.Field = function(n, v, m) {
        ...
        return this.fields.addField(n, v, this, m);
    }
...
Class.forName = function(name) { 
    ...
    //!!!!!! infinite cache !!!!
    if (!Class.$cached.hasOwnProperty(name)) { 
        Class.$cached[name] = eval(name);
        Class.$cached[name].$name = name;
    }
    return Class.$cached[name];
}
...
//!!!!
IllegalArgumentException = zebra.IllegalArgumentException;
