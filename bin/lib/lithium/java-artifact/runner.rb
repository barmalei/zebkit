...
class RunJavaCode < JavaFileRunner
    ...
    def run() 
        return exec4(cmd(), fetch_class_name()) 
    end
...
class RunJAR < JavaFileRunner
    ...
    def run()
        ...
        return exec4(command)
    end
end

class RunGroovyScript < FileCommand
    required JAVA
    
    attr_reader :groovy_home
    
    def initialize(name)
        super
        @groovy_home = "#{$lithium_home}/tools/groovy"
        raise "Cannot find groovy home" if !File.exists?(@groovy_home)
    end
    
    def build() 
        command = File.join(@groovy_home, "bin", "groovy")
        raise 'Run failed' if exec4("#{@groovy_home}/bin/groovy",  "#{fullpath()}", $arguments.join(' ')) != 0 
    end
