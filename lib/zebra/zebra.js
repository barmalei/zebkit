...
/**
 * Layout package provides number of classes, interfaces, methods and variables that allows developer easily 
 ...
 * @param  {zebra.layout.Layoutable} parent  a parent component 
 * @return {zebra.layout.Layoutable} an index of direct children component or -1 if no a children component can be found
 * @method getDirectAt
 * @for getDirectAt()
 */
...
/**
 * Get a top (the highest in component hierarchy) parent component of the given component 
...
/**
 * Translate the given relative to the specified component location in an absolute location. Absolute 
 * location is location relatively to the top parent component of the specified component.
 * @param  {zebra.layout.Layoutable} c a component
 * @return {zebra.layout.Layoutable}  a top parent component
...
pkg.Layoutable = Class(L, [
    function $prototype() {
        ...
        */
         ...
         *
         * @method find
         * @return {zebra.layout.Layoutable} found children component or null if no children component can be found
         */
         ...
         * lay outs its children components
         * @method laidout
         */
         ...
         * @return {Object} return size object the component wants to have as the following structure:
         ...
         * @return {zebra.layout.Layoutable} just added children layoutable component
         * @method insert
         */
         ...
         * @param  {zebra.layout.Layoutable} d a new children component to be set
         * @method set
         */
...
/**
 * Grid layout manager. can be used to split a component area to number of virtual cells where children 
 * components can be placed. The way how the children components have to be laid out in the cells can 
 * be customized by using "zebra.layout.Constraints" class:
        ...
        ctr.setPadding(8);
        // say the component has to be left aligned in a virtual cell of grid layout 
        ctr.ax = zebra.layout.LEFT;

        // create panel and set grid layout manager with two virtual rows and columns
        var p = new zebra.ui.Panel();
...
var instanceOf = zebra.instanceOf, L = zebra.layout, MB = zebra.util,
    ...
    MS = Math.sin, MC = Math.cos, $MMI = Math.min,  $fmCanvas = null, $fmText = null,
    $fmImage = null, $clipboard = null, $clipboardCanvas, $canvases = [],
    $density = typeof window.devicePixelRatio !== "undefined" ? window.devicePixelRatio
...
pkg.Bag = Class(zebra.util.Bag, [
    ...
    function loaded(v) {
        ...
        for (var k in v) {
            if (v.hasOwnProperty(k)) v[k] = this.loaded(v[k]);
        }
        return v;
    },
    ...
    function loadByUrl(url, context) {
        this.root = null;
        if (zebra.URL.isAbsolute(url) || context == null) {
...
var MDRAGGED = ME.DRAGGED, EM = null, MMOVED = ME.MOVED, MEXITED = ME.EXITED,
    ...
    $focusGainedCounter = 0, ME_STUB = new ME(null, ME.PRESSED, 0, 0, 0, 1);
     ...
     function $prototype() {
        ...
        */
        ...
        this.load = function(jsonPath) {
            jsonPath = jsonPath + (jsonPath.lastIndexOf("?") > 0 ? "&" : "?") +
        ...
        this.relocated = function(px,py){ pkg.events.performComp(CL.MOVED, this, px, py); };
        this.resized   = function(pw,ph){ pkg.events.performComp(CL.SIZED, this, pw, ph); };
        ...
        /**
         *  Set the UI component enabled state. Using this property an UI component 
 ...
 *  instance. The component uses the target view it wraps to calculate 
 *  its preferred size.
 *  @class ViewPan
 *  @constructor
 *  @extends {zebra.ui.Panel}
    ...
    function $prototype() {
        ...
        this.compShown = function(t){
            if (t.isVisible) this.repaint(t);
                ...
                if (t.parent != null) {
                    this.repaint(t.parent, t.x, t.y, t.width, t.height);
                }
            }
        };
        ...
        this.compSized = function(t, pw, ph){
            if (t.parent != null) {
                this.repaint(t.parent, t.x, t.y, 
                             (t.width  > pw) ? t.width  : pw, 
                             (t.height > ph) ? t.height : ph);
            }
        };
        ...
        this.paintComponent = function(g,c){
            ...
            if (c.border && c.border.outline && 
                b && c.border.outline(g, 0, 0, c.width, c.height,c)) 
            {
                g.save();
                g.clip();
 ...
 * It keeps current focus owner component and provides API to change 
...
pkg.CursorManager = Class(pkg.Manager, MouseListener, [
    function $prototype() {
        /**
         * Say which cursor has to be used for the given UI component. 
         * @param  {zebra.ui.Panel} t  an UI component 
         * @param  {zebra.ui.Cursorable} c a zebra.ui.Cursorable interface implementation. It has to 
         * provide "getCursorType(target,x,y)" method implementation that defines a cursor
         * type to be set for the given target object and location 
         * @method setCursorable
         */
...
pkg.EventManager = Class(pkg.Manager, [
    function $prototype() {
        ...
        /**
         * Register global event listener. The listener will get events according to event listeners 
         * interfaces it implements. For instance to listen key and mouse events the passed listener
         * has to be an instance of "zebra.ui.KeyListener" and "zebra.ui.MouseListener" interfaces:
...
/**
 *  Mouse wheel support class. Installs necessary mouse wheel listeners and handles mouse wheel events
 *  in Zebra UI.  The mouse wheel support is plugging that is configured by a json configuration. 
...
pkg.MouseWheelSupport = Class([
    ...
    /**
     * Setup mouse wheel support for the specified "zebra.ui.zCanvas" component
...
pkg.TextRender = Class(pkg.Render, zebra.util.Position.Metric, [
    function $prototype() {
         ...
         * @param  {Boolean} b 
         * @param  {Integer} off an offset starting from that the text has been updated 
         * @param  {Integer} size a size (in character) of text part that has been updated 
         * @param  {Integer} ful a first affected by the given update line
         * @param  {Integer} updatedLines a number of text lines that have been affected by text updating 
         * @method textUpdated
         */
 ...
 *  @class  Button
 *  @constructor
 *  @param {String|zebra.ui.Panel} [t] a button label. The label can be a simple text or an UI component.
...
pkg.Button = Class(pkg.StatePan, Actionable, [
    ...
    function $prototype() {
        ...
        /**
         * Set the mode the button has to fire events. Button can fire event after it has been unpressed 
         * or immediately when it has been pressed. Also button can start firing events periodically when it has been 
         * pressed and holden in the pressed state.
         * @param  {Boolean} b   true if the button has to fire event by pressed event
         * @param  {Integer} time the period of time the button has to repeat firing events if it has been 
         * pressed and holden in pressed state. -1 means event doesn't have repeated  
...
/**
 *  Border panel UI component. The component render titled border around the given children content UI component.
 ...
 *  @param {zebra.ui.Panel|String} [title] a border panel title. Can be a string or any other UI component can be 
 *  used as the border panel title
 *  @param {zebra.ui.Panel} [content] a content UI component of the border panel 
 *  @param {Integer} [constraints] a title constraints. The constraints gives a possibility to place border panel
 *  title in different places. Generally the title can be placed on the top or bottom part of the border panel.
 *  Also the title can be aligned horizontally. 
        
         // create border panel with a title located at the top and aligned at the canter   
 ...
 * how the box and label component s have to be laid out, which UI components have to be used as 
 ...
 *  @event valueUpdated 
 *  @param {zebra.ui.Progress} src a progress bar that triggers the event 
...
pkg.ScrollManager = Class([
    function $prototype() {
         ...
         * @param  {Integer} x a x coordinate of top left corner of the rectangular area
         * @param  {Integer} y a y coordinate of top left corner of the rectangular area
...
/**
 * Scroll UI panel. The component is used to manage scrolling for a children UI component that occupies more space than 
 * it is available.
 * @param {zebra.ui.Panel} [c] an UI component that has to be placed into scroll panel  
 * @param {Integer} [barMask] a scroll bars mask that allows developers to control vertical and 
...
pkg.Tabs = Class(pkg.Panel, MouseListener, KeyListener, [
    function $prototype() {
         ...
         * @return {Integer} an index of the tab that is detected at the given location. 
         * -1 if no any tab can be found
         * @method getTabAt
         */
...
pkg.configure(function(conf) {
    var p = zebra()["ui.json"];
    conf.loadByUrl(p ? p : "ui.json", pkg);
});
