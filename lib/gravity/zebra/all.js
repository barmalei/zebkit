...
pkg.Bag = Class(function($) {
    ...
    $(function () {
        ...
        this.aliases["Dimension"] = function(w, h) { this.width =w; this.height = h; };
        this.put("true", true);
...
pkg.BaseList = Class(pkg.Panel, pkg.MouseMotionListener, pkg.MouseListener, pkg.KeyListener,
    ...
    function (b){
        ...
        this.$super();
        this._ = new Listeners();
        this.views = { "select":pkg.get("list.sv"), "marker":pkg.get("list.pos.br") };
...
pkg.Combo = Class(pkg.Panel, pkg.MouseListener, pkg.KeyListener, pkg.Composite, pkg.FocusListener, [
    function $clazz() {
        this.ContentPan = Class(pkg.Panel, [
            function (){
                this.$super();
        ...
        this.Button = Class(pkg.Button, [
            function() {
                ...
                if(ps != null) this.setPreferredSize(ps.width, ps.height);
            }
        ]);
    },
...
pkg.Window = Class(pkg.Panel, pkg.MouseMotionListener, WinListener,
    ...
    function createCaption() {
        ...
        this.title.view.setDefBoldFont();
...
pkg.Menu = Class(pkg.CompList, pkg.ChildrenListener, [
    function $prototype() {
        this.getSubMenuIndex = function (target){
            for(var i = 0;i < this.menus.length; i++) if (this.menus[i][0] == target) return i;
            return -1;
        };

        this.isDecorative = function (index){
            return !(zebra.instanceOf(this.kids[index], pkg.Panel) && zebra.instanceOf(this.kids[index].layout, pkg.Menu));
        };
    ...
    function (key){
        ...
        this.views["marker"] = pkg.get("menu.pos.br");
        this.views["select"] = pkg.get("menu.sv");
        this.padding(3);
...
pkg.MenuBar = Class(pkg.Panel, pkg.ChildrenListener, pkg.KeyListener, [
    ...
    function (){
        ...
        this.views = { on:pkg.get("mbar.onbr"), off:pkg.get("mbar.offbr") };
...
pkg.Line = Class(pkg.Panel, [
    ...
    function getPreferredSize() { return { width:5, height:5 }; }
]);
            ...
            switch(e.code)
            {
                ...
                case KE.VK_RIGHT:
                    if(e.isControlPressed()){
                        var p = this.findNextWord(this.getModel(), line, col, 1);
...
pkg.Tree = Class(ui.Panel, ui.MouseListener, ui.KeyListener,
    ...
    function (d,b){
        ...
        this.views = {};
        this.viewSizes = {};
        this.setView("on"     , ui.get("tree.on"));
        this.setView("off"    , ui.get("tree.off"));
        this.setView("least"  , ui.get("tree.least"));
        this.setView("open"   , ui.get("tree.open"));
        this.setView("close"  , ui.get("tree.close"));
        this.setView("aselect", ui.get("tree.av"));
        this.setView("iselect", ui.get("tree.iv"));
    ...
    function setView(id, v){
        if(this.views[id] != v){
            this.views[id] = v;
            if(id != "aselect" && id != "iselect"){
                this.stopEditing(false);
                this.viewSizes[id] = (v == null) ? { width:0, height:0 } 
                                                 : v.getPreferredSize();
                this.vrp();
            }
        }
    ...
    function toggle(item){
        if(this.model.hasChildren(item)){
            ...
            this.invalidate();
            this._.fireTo("toggled", [this, item]);
    ...
    function select(item){
        if(this.isSelectable && item != this.selected){
            ...
            this._.fireTo("selected", [this, this.selected]);
...
var ScrollListener = ui.ScrollListener, TextRender = ui.view.TextRender, View = ui.view.View,
    ...
    Position = zebra.util.Position, KE = ui.KeyEvent, rgb = zebra.util.rgb,
...
pkg.DefViews = Class([
    ...
    function $prototype() {
        ...
        this.getCellColor  = function(row,col){ return this.defBgColor;},
...
pkg.GridCaption = Class(ui.Panel, ui.MouseMotionListener, ui.MouseListener, ui.Cursorable, [
            ...
            if(this.pxy != null){
                ...
                if (ns > this.minSize) {
                    if (b) {
                        ...
                        m.setColWidth (rc, ns);
                        this._.fireTo("captionColResized", [ this, rc, pw ]);
                    }
    ...
    function calcPreferredSize(l) { return { width:this.psW, height:this.psH }; },
...
pkg.Grid = Class(ui.Panel, ui.MouseListener, ui.KeyListener, Position.PositionMetric,
        ...
        function (data){
            ...
            this._ = new Listeners();
            this.views = { "onselection": ui.get("grid.asv"), "marker": ui.get("grid.mv"), "offselection":ui.get("grid.isv") };
        ...
        function rCustomMetric(){
            ...
            for(; start < this.colWidths.length; start ++ ) this.colWidths[start] = pkg.Grid.DEF_COLWIDTH;
            start = 0;
...
pkg.GridStretchPan = new Class(ui.Panel, zebra.layout.Layout, [
    ...
    function captionColResized(src, col, pw){
        var grid = this.getGrid();
...
var L = zebra.layout, View = ui.view.View, rgb = zebra.util.rgb, Wizard = ui.Wizard;

pkg.Wizard = Class(Wizard, [
    function() {
        var W = pkg.Wizard;
