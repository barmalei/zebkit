...
var MB = zebra.util, Point = zebra.util.Point, Bag = zebra.util.Bag, Rectangle = zebra.util.Rectangle, Composite = pkg.Composite;
var Dimension = zebra.util.Dimension, Color = zebra.util.Color, TextRender = pkg.view.TextRender, MouseListener = pkg.MouseListener;
    ...
    $(function() {
        ...
        this.startPaint = function(g,d){
            ...
            try {
                d.validate();
                var r = d.da;
                if (r.width > 0) g.clipRect(r.x, r.y, r.width, r.height);
                this.paint(g, optPaint(d, g), null);
            }
            finally {
                r.width = -1;
                if (g.restore() != id) ; //!!!!throw new Error(":::" + g.counter);
            }
        }
        
        this.paint = function(g,c,p){
            var dw = c.width, dh = c.height, ts = g.getTopStack();
            if(ts.width > 0 && ts.height > 0 && dw != 0 && dh != 0){
                c.validate();
                ...
                if(c_w > 0 && c_h > 0){
                    ...
                    for(var i = 0;i < count; i++){
                        ...
                        if(kid.isVisible){
                            ...
                            if(iw > 0 && ih > 0) this.paint(g, kid, c);
                        }
                    }
    ...
    $(function paint(g){
        ...
        if(this.orient == L.HORIZONTAL){
            ...
            if(this.isEnabled)
                ...
                g.setColor(Color.gray);
                g.strokeRect(leftX + Math.floor((bs.width - gs.width) / 2), top + 1, gs.width, h);
            }
            leftX += bs.width;
            if(this.isShowScale){
                leftX += this.gap;
