...
pkg.MouseListener        = new Interface();
pkg.MouseMotionListener  = new Interface();
pkg.FocusListener        = new Interface();
pkg.KeyListener          = new Interface();
pkg.ChildrenListener     = new Interface();
pkg.WinListener          = new Interface();
pkg.ScrollListener       = new Interface();
pkg.ExternalEditor       = new Interface();

pkg.ComponentListener = new Interface();
...
pkg.ContainerListener = new Interface();
var CNL = pkg.ContainerListener;
CNL.COMP_ADDED = 1;
CNL.COMP_REMOVED = 2;
CNL.LAYOUT_SET = 3;

pkg.InputEvent = Class(JAVA.util.EventObject, function($) {
    var IE = this;
    ...
    $(function (target, id){
        this.$super(target);
        if(id != IE.FOCUS_GAINED && id != IE.FOCUS_LOST) throw new IllegalArgumentException();
...
var InputEvent = pkg.InputEvent

pkg.KeyEvent = Class(InputEvent, function($) {
    var KE = this;
    ...
    $(function (target,id,code,ch,mask){
        this.getKeyType = function(){
            if(this.code == KE.VK_ENTER || this.code == KE.VK_SPACE) return KE.DO_KEY;
            if(this.code == KE.VK_ESCAPE) return KE.CANCEL_KEY;
            return -1;
        }
...
pkg.MouseEvent = Class(InputEvent, function($) {        
    var ME = this;
    ...
    $(function (target,id,ax,ay,mask,clicks){
        this.reset = function(target,id,ax,ay,mask,clicks){
            ...
            this.mask = mask;
            this.clicks = clicks;
            L.getRelLocation(ax, ay, L.getTop(target), target, this);
        }
...
var ME = pkg.MouseEvent;

pkg.Cursorable = new Interface();
pkg.Composite  = new Interface();
pkg.Desktop    = new Interface();
pkg.Layer      = new Interface();
pkg.PopupInfo  = new Interface();
pkg.Switchable = new Interface();

var MB = zebra.util, Point = JAVA.awt.Point, Bag = zebra.util.Bag, Rectangle = JAVA.awt.Rectangle, Composite = pkg.Composite;
var Dimension = JAVA.awt.Dimension, Color = JAVA.awt.Color, TextRender = pkg.view.TextRender, MouseListener = pkg.MouseListener;
var Cursor = JAVA.awt.Cursor, TextModel = zebra.data.TextModel, Render = pkg.view.Render;
var Actionable = zebra.util.Actionable, ViewSet = pkg.view.ViewSet, timer = zebra.util.timer, KeyListener = pkg.KeyListener;
var Cursorable = pkg.Cursorable, FocusListener = pkg.FocusListener, ChildrenListener = pkg.ChildrenListener;
var MouseMotionListener = pkg.MouseMotionListener, Layer = pkg.Layer, Listeners = zebra.util.Listeners;

pkg.getDesktop = function(c){
    c = L.getTop(c);
    return zebra.instanceOf(c, pkg.Desktop) ? c : null;
}
...
var temporary = new Rectangle(), ImageLoader = Class([
    function() { 
        this.images = []; 
        this.callback = null;
    },
    ...
    function run() {
        ...
        if (l > 0) {
            for(var i=0; i < l; i++)  {
                ...
                if (!img.complete) this.images.push(img);
            }
        }
    ...
    function ready(f) {
        if (this.images.length == 0) f();
        else this.callback = f;
    }
]);
     
pkg.INIT = function(themeId, themeConf, ready) {
    var il = new ImageLoader(), tf = new JAVA.io.TextFileReader(themeId + "/" + themeConf);
    pkg.$objects._.add(function(src, id, d) {
            if (id == Bag.OBJ_ADDED && d != null && zebra.instanceOf(d, pkg.view.ImgRender)) il.add(d.target);
        });
    
    pkg.$objects.load(tf.text);
	pkg.wizard = pkg.$objects.get("wizard");

    //!!!
	tf = new JAVA.io.TextFileReader("demo/demo.properties");
    pkg.$objects.load(tf.text);
    
    il.ready(function() { ready(); }); 
}
...
pkg.Panel = Class(L.Layoutable, [
    function() { 
        ...
        this.updateCashedPs = function(w,h){
            if(w >= 0) this.cachedWidth = w;
            if(h >= 0) this.cachedHeight = h;
        }
        this.$super(); 
    },
    ...
    function notifyRender(o,n){
        if(o != null && zebra.instanceOf(o, Render)) o.ownerChanged(null);
        if(n != null && zebra.instanceOf(n, Render)) n.ownerChanged(this);
    },

    function add(c){ this.insert(this.kids.length, null, c); },
    function insert(i,d) { this.insert(i, null, d); },
    ...
    function getComponentAt(xx,yy){
        //!!!!zebra.sync(Panel.temporary, function() {
    ...
    function contains(x,y) { return true; },
    ...
    function isInvalidatedByChild(c) { return true; },
    ...
    function update(g){
        if(this.bg != null && (this.parent == null || !this.bg.equals(this.parent.bg))) this.bg.paint(g, 0, 0, this.width, this.height, this);
    ...
    function getTop(){ return this.border != null ? this.calcInsets(L.TOP, this.top, this.border.getTop()) : this.top; },
    function getLeft(){ return this.border!= null ? this.calcInsets(L.LEFT, this.left, this.border.getLeft()) : this.left; },
    function getBottom(){ return this.border != null ? this.calcInsets(L.BOTTOM, this.bottom, this.border.getBottom()) : this.bottom; },
    function getRight(){ return this.border != null ? this.calcInsets(L.RIGHT, this.right, this.border.getRight()) : this.right; },
    ...
    function calcPreferredSize(l){ 
        var v = this.getView();
        return  v == null ? this.$super(this.calcPreferredSize,l) 
                          : v.getPreferredSize(); 
    },
    ...
    function calcInsets(type,ci,bi){ return ci > bi ? ci : bi; },
    function customize(id) { pkg.wizard.customize(id, this);  }
]);

pkg.Wizard = Class(function($) {
    this.TFIELD = 0;
    this.LIST = 1;
    this.COMBO = 2;
    this.BUTTON = 3;
...
pkg.BaseLayer = Class(pkg.Panel, Layer, [
    function (id){
        this.pfo = null;
        this.$super();
        if(id == null) throw new IllegalArgumentException("2");
        this.id = id;
    },
    ...
    function isLayerActive(){ return true;},
    function isLayerActiveAt(x,y){return true;},

    function layerMousePressed(x,y,m){},
    function layerKeyPressed(code,m){},
    function getFocusRoot() { return this; },
]);
...
pkg.Label = Class(pkg.Panel, [
    ...
    function (r){
        ...
        this.setView(zebra.instanceOf(r, TextModel) ? new TextRender(r) : r);
        this.padding(1);
...
pkg.InputEventPan = Class(pkg.Panel, MouseListener,  MouseMotionListener, KeyListener, function($) {
    ...
    $(function keyPressed(e){
        if(this.state != PRESSED_OVER && this.state != PRESSED_OUT && KE.DO_KEY == e.getKeyType()){
    ...
    $(function keyReleased(e){
        if(this.state == PRESSED_OVER || this.state == PRESSED_OUT){
            ...
            if (!this.isMCInside) this.updateState(OUT);
        }
    });

    $(function mouseEntered(e){
        this.isMCInside = true;
    ...
    $(function mousePressed(e){
        if(this.state != PRESSED_OVER && this.state != PRESSED_OUT && e.isActionMask()){
            this.updateState(this.isMCInside ? PRESSED_OVER : PRESSED_OUT);
        }
    });

    $(function mouseReleased(e){
        if((this.state == PRESSED_OVER || this.state == PRESSED_OUT) && e.isActionMask()){
            this.updateState(this.isMCInside ? OVER : OUT);
        }
    });

    $(function mouseDragged(e){
        if(e.isActionMask()){
            this.isMCInside = (e.x > 0 && e.y > 0 && e.x < this.width && e.y < this.height);
            var pressed = (this.state == PRESSED_OUT || this.state == PRESSED_OVER);
            if(this.isMCInside) this.updateState(pressed ? PRESSED_OVER : OVER);
...
pkg.ActivePan = Class(pkg.InputEventPan, Composite, FocusListener, function($) {
    ...
    $(function setAsFocusComponent(index){ this.focusComponent = (index >= 0) ? this.get(index) : null;});
    $(function setFocusComponent(c){ this.setFocusComponent(c, true); });

    $(function setFocusComponent(c,clearBg){
        if(this.focusComponent != c){
            if(this.focusComponent != null) this.remove(this.focusComponent);
            this.focusComponent = c;
            if(c != null){
                if(clearBg) this.focusComponent.setBackground(null);
                if(this.indexOf(this.focusComponent) < 0) this.add(this.focusComponent);
            }
        }
    });

    $(function setFocusMarker(c){
    ...
    $(function catchInput(child){ return true; });
    ...
    $(function kidRemoved(i,l){
        ...
        this.$super(this.kidRemoved,i, l);
    });

    $(function stateUpdated(o,n){
        this.$super(this.stateUpdated,o, n);
        if(zebra.instanceOf(this.border, ViewSet)) this.activateView(this.border, o, n);
        if(zebra.instanceOf(this.getView(), ViewSet)) this.activateView(this.getView(), o, n);
    });

    $(function activateView(set,o,n){
        if((n == IEP.OVER || n == IEP.DISABLED) && set.getView(ActivePan.IDS[n]) == null) n = IEP.OUT;
...
pkg.Button = Class(pkg.ActivePan, Actionable, function($) {
    ...
    $(function (t){
        ...
        if(t != null){
            ...
            if(pkg.get("bt.marker") != null) this.setFocusMarker(pkg.get("bt.marker"));
        }
...
pkg.BorderPan = Class(pkg.Panel, pkg.view.TitleInfo, function($) {
    ...
    $(function calcPreferredSize(target){
        var ps = this.center != null && this.center.isVisible ? this.center.getPreferredSize() : new Dimension(0,0);
        ...
        if(this.label != null && this.label.isVisible){
            ...
            this.label.setLocation((this.xAlignment == L.LEFT) ? left + INDENT 
                                                               : ((this.xAlignment == L.RIGHT) ? this.width - right - ps.width - INDENT 
                                                                                               :  Math.floor((this.width - ps.width) / 2)), (this.orient == L.BOTTOM) ? (this.height - bottom - ps.height) : top);
        }
    ...
    $(function setTitleAlignment(a){
        if(a != L.TOP && a != L.BOTTOM) throw new IllegalArgumentException("5");
        if(a != this.orient){
            this.orient = a;
            this.vrp();
        }
    ...
    $(function setXAlignment(a){
        if(a != L.LEFT && a != L.RIGHT && a != L.CENTER) throw new IllegalArgumentException("6");
    ...
    $(function calcInsets(type,ci,bi){ return type == this.orient && this.label != null ? 0 : bi; });
});
...
pkg.ImagePan = Class(pkg.Panel, [
    ...
    function (v){
        if (v) {
            if (zebra.isString(v)) v = pkg.get(v);
            if (v.constructor === Image)  v = new pkg.view.ImgRender(v);
        } 
        this.$super(); 
        this.setView(v);
        this.setBackground(null);
...
pkg.SwitchManager = Class(function($) {
    $(function (){ this._ = new Listeners(); });
    
    $(function install(o){o.switched(this.getState(o));});
    $(function uninstall(o){});
    $(function fire(s){ this._.fire(this, s); });

    this.Abstract(function getState(o){ });
    this.Abstract(function setState(o,b){ });
});

pkg.SwitchManImpl = Class(pkg.SwitchManager, [
    function () { 
        this.state = false;
        this.$super(); 
    },
...
pkg.Group = Class(pkg.SwitchManager, [
    ...
    function getState(o){ return o == this.selected; },

    function setSelected(s){
        if(s != null) this.setState(s, true);
        else this.clearSelected();
    },

    function setState(o,b){
        if(b && o != this.selected){
            this.clearSelected();
            this.selected = o;
            if(this.selected != null) this.selected.switched(true);
...
pkg.Checkbox = Class(pkg.ActivePan,pkg.Switchable, Actionable, function($) {
    ...
    $(function (c,type){
        ...
        for(var i = 0;i < IDS.length; i++) set.put(IDS[i], prefix.concat(IDS[i]));
        box.setView(set);
        this.setBox(box, true);
        this.type = type;
        this.setSwitchManager(new pkg.SwitchManImpl());
        this.padding(2);
        this.setBackground(null);
    });

    $(function keyPressed(e){
        if(zebra.instanceOf(this.manager, pkg.Group) && this.getState()){
            var code = e.code, d = 0;
            ...
            if(d != 0) {
                ...
                for(var i = idx + d;i < p.kids.length && i >= 0; i += d){
                    var l = p.kids[i];
                    if(l.isVisible && l.isEnabled && zebra.instanceOf(l, pkg.Checkbox) && l.manager == this.manager){
                        l.requestFocus();
                        l.setState(true);
                        break;
                    }
    ...
    $(function setSwitchManager(m){
        if(m == null) throw new IllegalArgumentException("7");
        if(this.manager != m){
    ...
    $(function getDefaultLayout(){
        ...
        return (l == null) ? this.$super(this.getDefaultLayout) : l;
    });
...
pkg.SplitPan = Class(pkg.Panel, MouseMotionListener, Composite, Cursorable, function($) {
    ...
    $(function (f,s,o){
        ...
        if(bps != null) bar.setPSSize(bps.width, bps.height);
    ...
    $(function setOrientation(o){
        if(o != L.VERTICAL && o != L.HORIZONTAL) throw new IllegalArgumentException("8");
...
pkg.Progress = Class(pkg.Panel, Actionable, function($) {
    ...
    $(function setOrientation(o){
        if(o != L.HORIZONTAL && o != L.VERTICAL) throw new IllegalArgumentException("10");
...
pkg.Link = Class(pkg.Button, Cursorable, [
    function(s){
        ...
        this.colors = (this.colors == null) ? [Color.blue, Color.darkBlue, Color.black, Color.blue, Color.gray]
                                            : this.colors.slice(0);
...
pkg.Toolbar = Class(pkg.Panel, ChildrenListener, function($) {
    ...
    $(function (orient,gap){
        ...
        if(orient != L.HORIZONTAL && orient != L.VERTICAL) throw new IllegalArgumentException("13");
    ...
    $(function setView(c,id,v){
        if(c != null){
            if (this.indexOf(c) < 0) throw new IllegalArgumentException("15");
    ...
    $(function addImage(img){
        ...
        pan.padding(this.topShift, this.leftShift + 2, this.bottomShift, this.rightShift+2);
        this.add(pan);
        return pan;
    });
    ...
    $(function paint(g){
        for(var i = 0;i < this.kids.length; i++){
            ...
            if(c.isVisible && !isDecorative(c)){
                ...
                if(zebra.instanceOf(c, pkg.Checkbox) && c.getState()) index = PRESSED;
    ...
    $(function calcPreferredSize(target){
        ...
        return new Dimension(b ? w + c * (this.leftShift + this.rightShift) 
                               : w + this.topShift + this.bottomShift, b ? h + this.leftShift + this.rightShift : h + c * (this.topShift + this.bottomShift));
    });

    $(function layout(t){
        var b = (this.orient == L.HORIZONTAL), x = t.getLeft(), y = t.getTop();
        var av = this.topShift + this.bottomShift, ah = this.leftShift + this.rightShift;
        var hw = b ? t.height - y - t.getBottom() : t.width - x - t.getRight();
        for(var i = 0;i < t.kids.length; i++){
            ...
            if(l.isVisible){
                ...
                if(b){
                    if (str) ps.height = hw;
                    l.setLocation(x + this.leftShift, y + Math.round((hw - ps.height) / 2));
...
pkg.PaintManager = Class(function($) {
    $(function() {
        this.repaint = function(c,x,y,w,h){
            ...
            if(w > 0 && h > 0 && c.isVisible){
                var desktop = pkg.getDesktop(c);
                if(desktop != null){
                    var p = L.getAbsLocation(x, y, c);
                    x = p.x;
                    y = p.y;
                    //!!!zebra.sync(PaintManager.LOCKER, function() {
                        var x2 = desktop.width, y2 = desktop.height;
                        if(x < 0) {
                            w += x;
                            x = 0;
                        }
        ...
        this.startPaint = function(g,d){
            ...
            if(r.width > 0) {
                ...
                if(r.x < cx || r.y < cy || r.x + r.width > cx + cw || r.y + r.height > cy + ch){
                    MB.unite(r.x, r.y, r.width, r.height, cx, cy, cw, ch, r);
                    g.setClip(r.x, r.y, r.width, r.height);
                }
            }
            this.paint(g, optPaint(d, g.clip), null);
            r.width = -1;
        }
        
        this.paint = function(g,c,p){
            var dw = c.width, dh = c.height;
            if(dw != 0 && dh != 0){
                c.validate();
                this.paintComponent(g, c, p);

                g.clipRect(0, 0, dw, dh);
                var cl = g.clip, c_x = cl.x, c_y = cl.y, c_w = cl.width, c_h = cl.height;

                if(c_w > 0){
                    var count = c.kids.length;
                    for(var i = 0;i < count; i++){
                        var kid = c.kids[i];
                        if(kid.isVisible){
                            var kidX = kid.x, kidY = kid.y;
                            var ix = Math.max(kidX, c_x), iw = Math.min(kidX + kid.width,  c_x + c_w) - ix;
                            var iy = Math.max(kidY, c_y), ih = Math.min(kidY + kid.height, c_y + c_h) - iy;
                            if(iw > 0 && ih > 0) {
    ...
    function optPaint(d,r){
        for(var i = d.kids.length; --i >= 0; ){
            var l = d.kids[i];
            if(l.kids.length > 0) {
                var c1 = l.getComponentAt(r.x, r.y), c2 = l.getComponentAt(r.x + r.width - 1, r.y + r.height - 1);
...
pkg.PaintManImpl = Class(pkg.PaintManager, CL, [
    function() {
        ...
        this.compSized = function(pw,ph,t){
            ...
            var mw = (width > pw) ? width : pw, mh = (height > ph) ? height : ph, p = t.parent;
            if(p != null) p.repaint(t.x, t.y, mw, mh);
        }
        ...
        this.paintComponent = function(g,c,parent){
            c.update(g);
            var left = c.getLeft(), top = c.getTop(), bottom = c.getBottom(), right = c.getRight();
            if(left + right + top + bottom > 0){
                var cw = g.clip.width, ch = g.clip.height;
                if(cw <= 0) return;
                var cx = g.clip.x, cy = g.clip.y, x1 = Math.max(cx, left), y1 = Math.max(cy, top);
                g.setClip(x1, y1, Math.min(cx + cw, c.width - right) - x1, 
                                  Math.min(cy + ch, c.height - bottom) - y1);
            }
            c.paint(g);
        }

        this.paintOnTop = function(g,c){ c.paintOnTop(g); }
    }
]);

pkg.FocusManager = Class(MouseListener, CL, CNL, KeyListener, function($) {
    $(function() {
        ...
        this.keyPressed = function(e){
            if(KE.VK_TAB == e.code){
                var cc = ff(e.source, e.isShiftPressed() ?  -1 : 1);
                if(cc != null) this.requestFocus(cc);
            }
        }

        this.findFocusable = function(c){ return (isFocusable(c) ? c : fd(c, 0, 1)); }
    });

    $(function requestFocus(c){
        if(c != this.focusOwner && (c == null || isFocusable(c))){
    ...
    function isFocusable(c){ return c.isEnabled && c.canHaveFocus();}

    function fd(t,index,d){
        if(t.kids.length > 0){
            var isNComposite = !zebra.instanceOf(t, Composite);
            for(var i = index;i >= 0 && i < t.kids.length; i += d){
                var cc = t.kids[i];
                if(cc.isEnabled && cc.isVisible && cc.width > 0 && cc.height > 0 && (isNComposite ||  !t.catchInput(cc)) && (cc.canHaveFocus() || (cc = fd(cc, d > 0 ? 0 : cc.kids.length - 1, d)) != null))
                    return cc;
            }
        }
        return null;
    ...
    function ff(c,d){
        var top = c;
        while (zebra.instanceOf(top, Layer) == false) top = top.parent;
        ...
        return fd(top, d > 0 ? 0 : top.kids.length - 1, d);
    }

    function freeFocus(ctx, t){ if(t == ctx.focusOwner) ctx.requestFocus(null);}
});

pkg.CursorManager = Class(MouseListener, MouseMotionListener, function($) {
    $(function(){ 
        this.cursors = {};
        ...
        this.mouseMoved   = function(e){ setCursorType1(this, e); }
        this.mouseEntered = function(e){ setCursorType1(this, e); }
        this.mouseExited  = function(e){ setCursorType2(this, "default", e.source); }
        this.mouseDragged = function(e){ setCursorType1(this, e); }
    });
    
    function setCursorType1(tt, e){
        var t = e.source, c = tt.cursors.hasOwnProperty(t) ? tt.cursors[t] : null;
        if(c == null && zebra.instanceOf(t, Cursorable)) c = t;
        setCursorType2(tt, ((c != null) ? c.getCursorType(t, e.x, e.y) :  "default"), t);
    }

    function setCursorType2(tt, type,t){
        if(tt.cursorType != type){
            var d = pkg.getDesktop(t);
            if(d != null){
                tt.cursorType = type;
                d.canvas.style.cursor = (tt.cursorType < 0) ? "default" : tt.cursorType;
            }
        }
    }
});

pkg.EventManager = Class(function($) {
    var IEHM = [], MUID = InputEvent.MOUSE_UID, KUID = InputEvent.KEY_UID;
    var CSIZED = CL.COMP_SIZED, CMOVED = CL.COMP_MOVED;
    var CENABLED = CL.COMP_ENABLED, CSHOWN = CL.COMP_SHOWN;
    ...
    function handleCompEvent(l,id,a1,a2,c){
        switch(id) {
            ...
            default: throw new IllegalArgumentException("1000");
        }
    }
    ...
    function handleContEvent(l,id,a1,a2,c){
        switch(id) {
            ...
            default: throw new IllegalArgumentException("1001");
        }
    }
    ...
    $(function(){
        ...
        this.performCont = function(id,p,constr,c){
            if (zebra.instanceOf(p, CNL)) handleContEvent(p, id, p, constr, c);
            for(var i = 0;i < this.cc_l.length; i++) handleContEvent(this.cc_l[i], id, p, constr, c);

            for(var t = p.parent;t != null; t = t.parent){
                if(t.childContEvent && zebra.instanceOf(t, ChildrenListener)) t.childContEvent(id, p, constr, c);
            }
        }

        this.performComp = function(id,pxw,pxh,src){
            if(zebra.instanceOf(src, CL)) handleCompEvent(src, id, pxw, pxh, src);
            for(var i = 0;i < this.c_l.length; i++) handleCompEvent(this.c_l[i], id, pxw, pxh, src);
            for(var t = src.parent;t != null; t = t.parent){
                if(t.childCompEvent && zebra.instanceOf(t, ChildrenListener)) t.childCompEvent(id, src);
            }
        }
        ...
        this.performInput = function(e){
            ...
            switch(e.UID)
            {
                case MUID:{
                    if(id > 10){
                        if (zebra.instanceOf(t, MouseMotionListener)) { 
                            var m = t[k];
                            if (m) m.call(t, e); 
                        }
                        it = this.mm_l;
                        ...
                        if(zebra.instanceOf(t, MouseListener)) { 
                            var m = t[k];
                            if (m) m.call(t, e); 
                        }
                        it = this.m_l;
                ...
                case KUID:{
                    if(zebra.instanceOf(t, KeyListener)) { 
                        var m = t[k];
                        if (m) m.call(t, e);
                    }
                    it = this.k_l;
                ...
                case InputEvent.FOCUS_UID:{
                    if(zebra.instanceOf(t, FocusListener)) { 
                        var m = t[k];
                        if (m) m.call(t, e); 
                    }
                    it = this.f_l;
                } break;
                default: throw new IllegalArgumentException("1002");
            }
            ...
            for (t = t.parent;t != null; t = t.parent){
                if (t.childInputEvent && zebra.instanceOf(t, ChildrenListener)) t.childInputEvent(e);
            }
        }
    });

    $(function addListener(l){
        if(zebra.instanceOf(l,CL))   this.addComponentListener(l);
        if(zebra.instanceOf(l,CNL))   this.addContainerListener(l);
        if(zebra.instanceOf(l,MouseListener))       this.addMouseListener(l);
        if(zebra.instanceOf(l,MouseMotionListener)) this.addMouseMotionListener(l);
        if(zebra.instanceOf(l,KeyListener))         this.addKeyListener(l);
        if(zebra.instanceOf(l,FocusListener))       this.addFocusListener(l);
    });

    $(function removeListener(l) {
        if(zebra.instanceOf(l, CL))   this.removeComponentListener(l);
        if(zebra.instanceOf(l, CNL))   this.removeContainerListener(l);
        if(zebra.instanceOf(l, MouseListener))       this.removeMouseListener(l);
        if(zebra.instanceOf(l, MouseMotionListener)) this.removeMouseMotionListener(l);
        if(zebra.instanceOf(l, KeyListener))         this.removeKeyListener(l);
        if(zebra.instanceOf(l, FocusListener))       this.removeFocusListener(l);
    });
    ...
    function findComposite(t,child){
        ...
        var b = zebra.instanceOf(p, Composite), res = findComposite(p, b ? p : child);
        return (res != null) ? res : ((b && p.catchInput(child)) ? p : null);
    }
});

pkg.ScrollManager = Class(function($) {
    $(function (c){
        this._ = new Listeners('scrolled');
        this.target = c;
        this.targetIsListener = zebra.instanceOf(c, pkg.ScrollListener);
    });

    $(function getTargetComponent(){ return this.target; });
    ...
    $(function scrollTo(x,y){
        ...
        if(psx != x || psy != y){
            ...
            if (this.targetIsListener) this.target.scrolled(psx, psy);
            this._.fire(psx, psy);
        }
    });
    ...
    this.Abstract(function getSX(){ });
    this.Abstract(function getSY(){ });
    this.Abstract(function scrolled(sx,sy,psx,psy){ });
});

pkg.SimpleScrollManager = Class(pkg.ScrollManager, [
    function (c){ 
        this.sx = this.sy = 0;
        this.$super(c); 
    },
...
pkg.CompScrollManager = Class(pkg.SimpleScrollManager, L.Layout, [
    function (c){
        this.$super(c);
        this.tl = c.getLayout();
        c.setLayout(this);
    ...
    function scrolled(sx,sy,px,py){
        this.$super(this.scrolled, sx, sy, px, py);
        this.getTargetComponent().invalidate();
    }
]);
...
pkg.Scroll = Class(pkg.Panel, MouseListener, MouseMotionListener, 
    ...
    $(function (t) {
        if (t != L.VERTICAL && t != L.HORIZONTAL) throw new IllegalArgumentException("1");
    ...
    $(function layout(target){
        ...
        if(this.bundle != null && this.bundle.isVisible){
            ...
            if(am > MIN_BUNDLE_SIZE){
                var bsize = Math.max(Math.min(Math.floor((this.extra * am) / this.max), am - MIN_BUNDLE_SIZE), MIN_BUNDLE_SIZE);
    ...
    function pixel2value(s, p){
        ...
        return (s.type == L.VERTICAL) ? Math.floor((s.max * (p - db.y - db.height)) / (amount(s) - bn.height))
                                           : Math.floor((s.max * (p - db.x - db.width )) / (amount(s) - bn.width));
    }

    function value2pixel(s){
        var db = s.decBt, bn = s.bundle, off = s.position.offset;
        return (s.type == L.VERTICAL) ? db.y + db.height +  Math.floor(((amount(s) - bn.height) * off) / s.max) 
                                           : db.x + db.width  +  Math.floor(((amount(s) - bn.width) * off) / s.max);
    }
...
pkg.ScrollPan = Class(pkg.Panel, pkg.ScrollListener, function($) {
    ...
    var ContentPanel = Class(pkg.Panel,[
            function (c){
                this.$super();
                this.sman = new pkg.ScrollManager(c, function($) {
                    $(function(cc) { this.$super(cc); });
            ...
            function getDefaultLayout(){ return new L.RasterLayout(L.USE_PS_SIZE); }
    ]);
    ...
    $(function insert(i,ctr,c){
        if(ScrollPan.SCROLLED_EL == ctr && c.getScrollManager() == null) c = new ContentPanel(c);
        this.$super(this.insert,i, ctr, c);
    });

    $(function kidAdded(index,id,comp){
        this.$super(this.kidAdded,index, id, comp);
    ...
    $(function kidRemoved(index,comp){
        this.$super(this.kidRemoved,index, comp);
    ...
    $(function layout(target){
        ...
        var right = this.getRight(), top = this.getTop(), bottom = this.getBottom(), left = this.getLeft();
        var ww = this.width - left - right, maxH = ww, hh = this.height - top - bottom, maxV = hh;
        var so = this.scrollObj.getPreferredSize();
...
pkg.Tabs = Class(pkg.Panel, MouseListener, KeyListener, pkg.view.TitleInfo, FocusListener, MouseMotionListener, function($) {
    ...
    $(function (o){
        ...
        this.brSpace = this.upperSpace = this.orient = this.vgap = this.hgap = this.tabAreaX = this.tabAreaY = this.tabAreaWidth = this.tabAreaHeight = 0;
        ...
        this.mouseMoved = function(e) {
            ...
            if (this.overTab != i) {
                ...
                this.overTab = i;
                if (this.views[TAB_OVER] != null) this.repaint(this.tabAreaX, this.tabAreaY, this.tabAreaWidth, this.tabAreaHeight);
            }
        }
    ...
    $(function setTitleAlignment(o){
        if(o != L.TOP && o != L.BOTTOM && o != L.LEFT && o != L.RIGHT) throw new IllegalArgumentException();
        if(this.orient != o){
            this.orient = o;
            this.vrp();
        }
    ...
    $(function isPageEnabled(index){ return this.kids[index].isEnabled; });

    $(function enablePage(i,b){
        var c = this.kids[i];
        if(c.isEnabled != b){
            c.setEnabled(b);
    ...
    $(function addPage(title,c){
        if(title != null){
            this.pages.push(title);
            this.pages.push(new Rectangle());
        }
        this.add(c);
    ...
    $(function insert(index,constr,c){
        if(Math.floor(this.pages.length / 2) <= this.kids.length){
            this.pages.push("Page " + index);
            this.pages.push(new Rectangle());
        }
        this.$super(this.insert,index, constr, c);
    });
    ...
    $(function recalc(){
        ...
        if(count > 0){
            ...
            var hadd = 2 * this.hTabGap + bv.getLeft() + bv.getRight();
            var vadd = 2 * this.vTabGap + bv.getTop() + bv.getBottom();
            for(var i = 0;i < count; i++){
    ...
    $(function paint(g){
        ...
        for(var i = this.selectedIndex + 1;i < Math.floor(this.pages.length / 2); i++) this.paintTab(g, i);
    ...
    $(function mousePressed(e){
        if(e.isActionMask()){
            ...
            if(index >= 0 && this.isPageEnabled(index)) this.select(index);
        }
    });
    ...
    $(function layout(target){
        ...
        var count = Math.floor(this.pages.length / 2), sp = 2*this.sideSpace;
        var xx = b ? (this.tabAreaX + this.sideSpace) 
                   : ((this.orient == L.LEFT) ? (this.tabAreaX + this.upperSpace) : this.tabAreaX + 1);
        var yy = b ? (this.orient == L.TOP ? this.tabAreaY + this.upperSpace : this.tabAreaY + 1) 
        ...
        for(var i = 0;i < count; i++){
            ...
            if(i == this.selectedIndex){
                if(b) {
                    l.setSize(this.width - left - right - 2 * this.hgap, this.height - this.tabAreaHeight - top - bottom - 2 * this.vgap);
                    l.setLocation(left + this.hgap, ((this.orient == L.TOP) ? top + this.tabAreaHeight : top) + this.vgap);
                }
                else {
                    l.setSize(this.width - this.tabAreaWidth - left - right - 2 * this.hgap, this.height - top - bottom - 2 * this.vgap);
                    l.setLocation(((this.orient == L.LEFT) ? left + this.tabAreaWidth : left) + this.hgap, top + this.vgap);
                }
            }
            else l.setSize(0, 0);
        }
    ...
    $(function getTitleBounds(){
        ...
        var res = b ? new Rectangle(this.tabAreaX, 0, this.tabAreaWidth, 0) 
    ...
    $(function setSize(w,h){
        if(this.width != w || this.height != h){
            ...
            this.$super(this.setSize, w, h);
        }
    });

    $(function getTabView(index){
        var data = this.pages[2 * index];
        if(zebra.instanceOf(data, pkg.view.View)) return data;
    ...
    $(function calcInsets(type,ci,bi){ return type == this.orient ? 0 : bi; });
    ...
    $(function paintTab(g, pageIndex){
        ...
        var ps = v.getPreferredSize(), px = b.x + L.getXLoc(ps.width, L.CENTER, b.width);
        var py = b.y + L.getYLoc(ps.height, L.CENTER, b.height);
    ...
    function next(t, page, d){
        for(; page >= 0 && page < Math.floor(t.pages.length / 2); page += d) if (t.isPageEnabled(page)) return page;
        return -1;
    }
});

pkg.Slider = Class(pkg.Panel,KeyListener, MouseListener, FocusListener,MouseMotionListener, 
                   pkg.view.ViewProvider, Actionable, function($) {
    var Slider = this;
    ...
    $(function setOrientation(o){
        if(o != L.HORIZONTAL && o != L.VERTICAL) throw new IllegalArgumentException();
        if(o != this.orient){
            this.orient = o;
    ...
    $(function setValue(v){
        if(v < this.min || v > this.max) throw new IllegalArgumentException();
        var prev = this.value;
        if(this.value != v){
            this.value = v;
    ...
    $(function paint(g){
        ...
        var bs = this.views[Slider.BUNDLE].getPreferredSize(), gs = this.views[Slider.GAUGE].getPreferredSize();
        var w = this.width - left - right - 2, h = this.height - top - bottom - 2;
        if(this.orient == L.HORIZONTAL){
            var topY = top + Math.floor((h - this.psH) / 2) + 1, by = topY;
            if(this.isEnabled)
                this.views[Slider.GAUGE].paint(g, left + 1, topY + Math.floor((bs.height - gs.height) / 2), w, gs.height, this);
            else{
                g.setColor(Color.gray);
                g.drawRect(left + 1, topY + Math.floor((bs.height - gs.height) / 2), w, gs.height);
            }
            topY += bs.height;
            if(this.isShowScale){
                topY += this.gap;
                g.setColor(this.isEnabled ? this.scaleColor : Color.gray);
            ...
            this.views[Slider.BUNDLE].paint(g, this.getBundleLoc(this.value), by, this);
        }
        else{
            var leftX = left + Math.floor((w - this.psW) / 2) + 1, bx = leftX;
            if(this.isEnabled)
                this.views[Slider.GAUGE].paint(g, leftX + Math.floor((bs.width - gs.width) / 2), top + 1, gs.width, h, this);
            else{
                g.setColor(Color.gray);
                g.drawRect(leftX + Math.floor((bs.width - gs.width) / 2), top + 1, gs.width, h);
            }
            leftX += bs.width;
            if(this.isShowScale){
                leftX += this.gap;
            ...
            this.views[Slider.BUNDLE].paint(g, bx, this.getBundleLoc(this.value), this);
        }
    ...
    $(function value2loc(v){
        return Math.floor((getScaleSize.call(this) * (v - this.min)) / (this.max - this.min)) + getScaleLocation.call(this);
    });
    ...
    $(function calcPreferredSize(l){ return new JAVA.awt.Dimension(this.psW + 2, this.psH + 2); });

    $(function recalc(){
        var ps = this.views[Slider.BUNDLE].getPreferredSize(), ns = this.isShowScale ? (this.gap + 2 * this.netSize) : 0;
        var dt = this.max - this.min, hMax = 0, wMax = 0;
    ...
    function paintNums(g,loc){
        if(this.isShowTitle)
            for(var i = 0;i < this.pl.length; i ++ ){
                ...
                if(this.orient == L.HORIZONTAL) render.paint(g, this.pl[i] - Math.floor(d.width / 2), loc, this);
                else render.paint(g, loc, this.pl[i] - Math.floor(d.height / 2), this);
            }
    }
    ...
    function getScaleLocation(){
        var bs = this.views[Slider.BUNDLE].getPreferredSize();
        return (this.orient == L.HORIZONTAL ? this.getLeft() + Math.floor(bs.width / 2) 
                                                 : this.getTop()  + Math.floor(bs.height/ 2)) + 1;
    }
    ...
    $(function getBundleBounds(v){
        var bs = this.views[Slider.BUNDLE].getPreferredSize();
        return this.orient == L.HORIZONTAL ? new Rectangle(this.getBundleLoc(v), this.getTop() + Math.floor((this.height - this.getTop() - this.getBottom() - this.psH) / 2) + 1, bs.width, bs.height) 
                                           : new Rectangle(this.getLeft() + Math.floor((this.width - this.getLeft() - this.getRight() - this.psW) / 2) + 1, this.getBundleLoc(v), bs.width, bs.height);
    });
