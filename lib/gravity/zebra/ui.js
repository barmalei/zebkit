...
pkg.$objects = new zebra.util.Bag([
    function() { 
        ...
        for (var i = 0; i < pkgs.length; i++) {
            for(var k in pkgs[i]) {
                var v = pkgs[i][k];
                if (v && zebra.instanceOf(v, Class)) this.aliases[k] = v;
            }
        }
...
pkg.load = function(path) { pkg.$objects.load((new zebra.io.TextFileReader(path)).text); }
...
zebra.ready(function() {    
    ...
    var t = zebra.env["ui.theme"], p = zebra.env["ui.palette"];
    if (!t) t = "p3d";
    if (!p) p = "default";    
    pkg.load("theme/" + t + "/" + p + ".properties");
});
