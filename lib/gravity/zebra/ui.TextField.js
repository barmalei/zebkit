...
var ME = pkg.MouseEvent, KE = pkg.KeyEvent, PO = zebra.util.Position, P = JAVA.awt.Point, Color = JAVA.awt.Color;

pkg.TextPosition = Class(PO, zebra.data.TextModelListener, [
    function (render){
        this.$super(render);
        render.target.addTextModelListener(this);
    },
    ...
    function destroy(){ this.metrics.target.removeTextModelListener(this); }
]);    
...
pkg.TextField = Class(pkg.Label, pkg.KeyListener, pkg.MouseListener, 
    ...
    $(function (render){
        ...
        if (this.curColor == null) this.curColor = Color.black;
    ...
    $(function setPSByRowsCols(r,c){
        ...
        var h = (r > 0) ? (r * tr.getLineHeight() + (r - 1) * tr.getLineIndent()) : this.psHeight;
        this.setPSSize(w, h);
    });
    ...
    $(function getCursorType(target,x,y){ return JAVA.awt.Cursor.TEXT; });
    ...
    $(function posChanged(target,po,pl,pc){
        ...
        if(position.offset >= 0){
            var lineHeight = this.getView().getLineHeight(), top = this.getTop();
...
//!!!                g.setColor(this.getForeground());
    ...
    $(function pageSize(){
        ...
        var render = this.getView(), indent = render.getLineIndent(), textHeight = render.getLineHeight();
