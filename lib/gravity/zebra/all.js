    ...
    f.Import = function() {
        ...
        f(function(n, p) {
            if (packages == null || packages.indexOf(n) >= 0) {
                for (var k in p) {
                    if ((p[k] instanceof Package) == false) { 
...
(function(pkg, Class) {
    ...
    pkg.readTextFile =  function(path) {
        ...
        req.send(null);
        if (req.status != 200) throw new Error(path);
    	...
    	function(container) {
    	    if (isBA && container instanceof ArrayBuffer) this.data = new Uint8Array(container);
        	    ...
        	    if (zebra.isString(container)) {
            	    this.Field("read", function() { return this.available() > 0 ? this.data.charCodeAt(this.pos++) & 0xFF : -1; });
        	    }
        	    else {
...
pkg.Properties = Class([
	...
	function load(reader) {
    	...
    	finally { try { reader.close(); } catch(e) {} }
	}
]);
...
pkg.ListModelListener = Interface();
pkg.ListModel = Interface();
    
pkg.TextModelListener = Interface();
pkg.TextModel = Interface();

pkg.TreeModelListener = Interface();
pkg.TreeModel = Interface();

pkg.MatrixModel = Interface();
pkg.MatrixModelListener = Interface();

var TML = pkg.TextModelListener, MB = zebra.util;
TML.REMOVED  = 1;
TML.INSERTED = 2;

var Listeners = MB.Listeners;
...
pkg.TreeModelImpl = Class(pkg.TreeModel, function($) {
    var TreeModelImpl = this, INSERTED = 1, REMOVED = 2, MODIFIED = 3, MOVED = 4, Item = pkg.Item;
    ...
    $(function() { this.$this(new pkg.Item()); });
    ...
    $(function clone(item){
        var root = new Item(item), res = new TreeModelImpl(root);
    ...
    function pclone(res,root,d){
        for(var i = 0;i < d.kids.length; i++){
            var originalItem = d.kids[i], item = new pkg.Item(originalItem);
            res.add(root, item);
...
pkg.Matrix = Class(pkg.MatrixModel,function($) {
    $(function (rows,cols) { 
...
var Dimension = zebra.util.Dimension, Point = zebra.util.Point;
...
pkg.Layoutable = Class(L, [
    ...
    function setLayout(m){
        if (!m) throw new Error("Layout has to be defined"); 
...
pkg.MouseEvent = Class(InputEvent, function($) {        
    ...
    $(function (target,id,ax,ay,mask,clicks){
        this.reset = function(target,id,ax,ay,mask,clicks){
            ...
            this.mask = mask;
            this.clicks = clicks;
            L.getRelLocation(ax, ay, L.getTopParent(target), target, this);
        }
...
var MB = zebra.util, Point = zebra.util.Point, Rectangle = zebra.util.Rectangle, Composite = pkg.Composite;
var Dimension = zebra.util.Dimension, rgb = zebra.util.rgb, TextRender = pkg.view.TextRender, MouseListener = pkg.MouseListener;
var Cursor = pkg.Cursor, TextModel = zebra.data.TextModel, Render = pkg.view.Render, temporary = new Rectangle();
...
pkg.Bag = Class(zebra.util.Bag,[
    ...
    function load(path, context) {
        ...
        this.load(new zebra.io.InputStreamReader(path.toString()));
    }
]);
...
pkg.Panel = Class(L.Layoutable, [
    ...
    function notifyRender(o,n){
        if(o != null && instanceOf(o, Render)) o.ownerChanged(null);
        if(n != null && instanceOf(n, Render)) n.ownerChanged(this);
    },
    ...
    function getTop()   { return this.border != null ? this.top    + this.border.getTop()    : this.top;    },
    function getLeft()  { return this.border != null ? this.left   + this.border.getLeft()   : this.left;   },
    function getBottom(){ return this.border != null ? this.bottom + this.border.getBottom() : this.bottom; },
    function getRight() { return this.border != null ? this.right  + this.border.getRight()  : this.right;  },
                        ...
                        if (da.width > 0 && !$timers[desktop]) {
                            ...
                            $timers[desktop] = setTimeout(function() { 
                                try {
                                    ...
                                    $this.paint(context, optPaint(desktop, context));
                                    context.restore();
                                    da.width = -1; //!!!
                                }
...
pkg.FocusManager = Class(pkg.Manager, MouseListener, CL, CNL, KeyListener, function($) {
    ...
    function fd(t,index,d){
        if(t.kids.length > 0){
            var isNComposite = !instanceOf(t, Composite);
...
pkg.EventManager = Class(pkg.Manager, function($) {
    ...
    function findComposite(t,child){
        ...
        var b = instanceOf(p, Composite), res = findComposite(p, b ? p : child);
...
pkg.Tabs = Class(pkg.Panel, MouseListener, KeyListener, pkg.view.TitleInfo, FocusListener, MouseMotionListener, function($) {
    ...
    function next(t, page, d){
        for(; page >= 0 && page < ~~(t.pages.length / 2); page += d) if (t.isPageEnabled(page)) return page;
        return -1;
    }
});
...
pkg.Slider = Class(pkg.Panel,KeyListener, MouseListener, FocusListener,MouseMotionListener, 
    ...
    $(function getBundleBounds(v){
        ...
        return this.orient == L.HORIZONTAL ? new Rectangle(this.getBundleLoc(v), 
                                                           this.getTop() + ~~((this.height - this.getTop() - this.getBottom() - this.psH) / 2) + 1, bs.width, bs.height) 
                                           : new Rectangle(this.getLeft() + ~~((this.width - this.getLeft() - this.getRight() - this.psW) / 2) + 1, this.getBundleLoc(v), bs.width, bs.height);
    });
...
pkg.BaseList = Class(pkg.Panel, pkg.MouseMotionListener, pkg.MouseListener, pkg.KeyListener,
                         Position.PositionMetric, pkg.ScrollListener, pkg.FocusListener, pkg.view.ViewProvider, 
                         zebra.data.ListModelListener, function($) {
    var BaseList = this;
    ...
    $(function createModel(){ return new zebra.data.List(); });
...
pkg.CompList = Class(pkg.BaseList, pkg.Composite, zebra.data.ListModel, [
    function (){ this.$this(false); },
...
var WinListener = pkg.WinListener, ComponentListener = pkg.ComponentListener, Dimension = zebra.util.Dimension;
...
pkg.WinLayer = Class(pkg.BaseLayer, pkg.ChildrenListener, function($) {
    ...
    $(function (){
        this.support = this.activeWin = null;
        ...
        this.layerMousePressed = function(x,y,mask){
            ...
            if(cnt > 0){
                if(this.activeWin != null && this.indexOf(this.activeWin) == cnt - 1){
                    ...
                    var x2 = x1 + this.activeWin.width, y2 = y1 + this.activeWin.height;
    ...
    $(function addWinListener(l){
        if(this.support == null) this.support = [];
        if(this.support.indexOf(l) < 0) this.support.push(l);
    });

    $(function removeWinListener(l){
        if(this.support != null && this.support.indexOf(l) >= 0) zebra.util.array_rm(this.support,l);
    });
    ...
    $(function add(c,l,type){ return this.add([type, l], c); });

    $(function kidAdded(index,id,lw){
        this.$super(this.kidAdded,index, id, lw);
        if(id == null) id = WL.MDI;
        var info = [null, null, null];
        info[0] = this.activeWin;

        if(zebra.isNumber(id)) info[1] = id;
        else
            if(zebra.instanceOf(id, WinListener)) info[2] = id;
            else {
                info[1] = id[0];
                info[2] = id[1];
            }

        if(info[1] == null) info[1] = WL.MDI;
    ...
    $(function kidRemoved(index,lw){
        ...
        var ci = this.winsStack.indexOf(lw), l = winListener(this, lw);
    ...
    $(function activate(c){
        ...
        if(c != this.activeWin){
            if(c == null){
                if (winType(this, this.activeWin) == WL.MODAL) throw new Error("102");
                var old = this.activeWin;
                this.activeWin = null;
                pw(this, winListener(this, old), WIN_DEACTIVATED, old);
                pkg.focusManager.requestFocus(null);
            }
            else{
                if(this.winsStack.indexOf(c) < this.topModalIndex) throw new Error("103");
                var old = this.activeWin;
                this.activeWin = c;
                this.toFront(this.activeWin);
                if(old != null) pw(this, winListener(this, old), WIN_DEACTIVATED, old);
                pw(this, winListener(this, this.activeWin), WIN_ACTIVATED, this.activeWin);
    ...
    function winListener(t, c){ return (t.winsInfo[c])[2]; }

    function pw(t, l,id,target){
        if(this.support != null) for(var i = 0;i < t.support.length; i++) process(t.support[i], id, target);
        if(l != null) process(l, id, target);
        if(zebra.instanceOf(target, WinListener)) process(target, id, target);
    }

    function process(l,id,target){
        if(id == WIN_OPENED || id == WIN_CLOSED) l.winOpened(this, target, id == WIN_OPENED);
        else l.winActivated(this, target, id == WIN_ACTIVATED);
    }
});

pkg.Window = Class(pkg.Panel, pkg.MouseMotionListener, pkg.WinListener,
    ...
    this.show = function(canvas, win, modal) {
        canvas.getLayer(pkg.WinLayer.ID).add(modal === true ? WinLayer.MODAL : WinLayer.MDI, win);
    }

    this.hide = function(win) { win.parent.remove(win); }
    
    $(function (){  this.$this("");  });
    $(function (s){ this.$this(pkg.get("win.prod"), s); });
    ...
    $(function winOpened(winLayer,target,b) { 
        if (this.caption != null) this.caption.setBackground(pkg.get("win.cap.iv")); 
    });
...
pkg.TooltipManager = Class(pkg.Manager, pkg.MouseListener, pkg.MouseMotionListener, function($) {
        ...
        this.run = function(){
            if(this.tooltip == null){
                ...
                if(this.tooltip != null) {
                    ...
                    var tx = p.x, ty = p.y - this.tooltip.height, dw = this.targetLayer.width;
                    if(tx + ps.width > dw) tx = dw - ps.width - 1;
                    this.tooltip.setLocation(tx < 0 ? 0 : tx, ty);
                    this.targetLayer.add(pkg.WinLayer.INFO, this.tooltip);
                }
            }
        }
    });
    ...
    this.createTooltip = function(text){
        ...
        if(ins != null) lab.padding(ins[0], ins[1], ins[2], ins[3]);
...
pkg.Menu = Class(pkg.CompList, pkg.ChildrenListener, function($) {
    ...
    $(function (key){
        ...
        this.setElementView(Menu.POS_BR_VIEW, pkg.get("menu.pos.br"));
        this.setElementView(Menu.SEL_VIEW, pkg.get("menu.sv"));
        this.padding(3);
    ...
    $(function calcPreferredSize(target){
        var cc = target.kids.length, gaps = this.getItemGaps();
    ...
    $(function update(g){
        this.$super(this.update,g);
        if(this.getElementView(Menu.POS_BR_VIEW) != null && this.hasFocus()){
            var gap = this.getItemGaps(), offset = this.position.offset;
            if(offset >= 0 && !isDecorative(this, offset)){
                this.getElementView(Menu.POS_BR_VIEW).paint(g, this.getItemX(offset) - gap, 
                                                               this.getItemY(offset) - gap, 
                                                               this.getItemW(offset) + 2 * gap, 
        ...
        if(position.metrics.getMaxOffset() > 0){
            ...
            if(code == KE.VK_DOWN){
                ...
                if(code == KE.VK_UP){
                    ...
                    if(e.getKeyType() == KE.DO_KEY) this.select(offset);
        }
    });

    $(function drawPosMarker(g,x,y,w,h){});
    $(function getDefaultLayout(){return new L.ListLayout(1);});
...
pkg.MenuBar = Class(pkg.Panel, pkg.ChildrenListener, pkg.KeyListener, pkg.Switchable, function($) {
    ...
    $(function childInputEvent(e){
        ...
        switch(e.ID)
        {
            ...
            case MouseEvent.EXITED:{
                var p = {};
                L.getRelLocation(e.absX, e.absY, pkg.getDesktop(this), this.over, p);
                if(p.x < 0 || p.y < 0 || p.x >= this.over.width || p.y >= this.over.height){
                    var prev = this.over;
                    this.over = null;
    ...
    $(function getDefaultLayout(){return new L.FlowLayout(4);});
            ...
            if(d != null){
                ...
                if(t.selected != null){
                    ...
                    if(menu != null && menu.hasVisibleItems()){
                        ...
                        menu.setLocation(abs.x, abs.y + t.selected.height + 1);
                        pop.add(menu);
                    }
                }
...
pkg.PopupLayer = Class(pkg.BaseLayer, pkg.ChildrenListener, function($) {
    ...
    $(function childInputEvent(e){
        if(e.UID == pkg.InputEvent.KEY_UID){
            if(e.ID == KE.PRESSED && e.getKeyType() == KE.CANCEL_KEY){
                var p = e.source;
    ...
    $(function doLayout(target){
        ...
        for(var i = 0; i < cnt; i++){
            ...
            if(zebra.instanceOf(m, pkg.Menu)){
                ...
                var yy = (m.y + ps.height > this.height) ? this.height - ps.height : m.y;
                if(xx < 0) xx = 0;
                if(yy < 0) yy = 0;
                m.setLocation(xx, yy);
            }
...
var MDRAGGED = MouseEvent.DRAGGED, EM = null, MMOVED = MouseEvent.MOVED, MEXITED = MouseEvent.EXITED, MENTERED = MouseEvent.ENTERED;
...
pkg.zCanvas = Class(pkg.Panel, pkg.Desktop, function($) {
	var KE_STUB = new pkg.KeyEvent  (null,  KPRESSED, 0, 'x', 0);
  	var ME_STUB = new pkg.MouseEvent(null,  MouseEvent.PRESSED, 0, 0, 0, 1);
...
pkg.TextField = Class(pkg.Label, pkg.KeyListener, pkg.MouseListener, 
    ...
    function getTextRowColAt(render,x,y){
        ...
        var ln = (y < 0) ? 0 : ~~((y + li) / (lh + li)) + ((y + li) % (lh + li) > li ? 1 : 0) -1;
...
var KE = ui.KeyEvent, Dimension = zebra.util.Dimension, Rectangle = zebra.util.Rectangle, rgb = zebra.util.rgb;
...
pkg.Tree = Class(ui.Panel, zebra.data.TreeModelListener, ui.view.ViewProvider, ui.MouseListener,
    ...
    function getImageBounds(t, root){
        ...
        return new Rectangle(node.x + td.width + (td.width > 0 ? t.gapx : 0), 
                             node.y + ~~((node.height - id.height) / 2), id.width, id.height);
    }
    ...
    function paintBranch(t, g,root){
        ...
        if(t.visibleArea.intersects(node.x + dx, node.y + dy, node.width, node.height)){
            ...
            var vy = node.y + ~~((node.height - node.viewHeight) / 2);
            ...
            if(t.lnColor != null){
                ...
                var x1 = toggle.x + (toggleView == null ? ~~(toggle.width / 2) + 1 : toggle.width) + 1
                var yy = toggle.y + ~~(toggle.height / 2);
    ...
    function y_(t, item,isStart){
        ...
        var y = (t.model.hasChildren(item)) ? (isStart ? ty + th : ty - 1) : ty + ~~(th / 2);
        if(y + dy < 0) y =  -dy - 1;
    ...
    function paintChild(t, g,root,index){
        ...
        if(root == t.firstVisible && t.lnColor != null){
            ...
            var xx = getIM(t, root).x + ~~((b ? vs[Tree.TOGGLE_ON_VIEW].width 
...
pkg.Grid = Class(ui.Panel, zebra.data.MatrixModelListener, ui.MouseListener, ui.KeyListener, Position.PositionMetric,
        ...
        $(function useTopCaption() { return this.useCaption(Grid.TOP_CAPTION_EL, null); });
        $(function useTopCaption(titles) { return this.useCaption(Grid.TOP_CAPTION_EL, titles); });
        
        $(function useCaption(constr, titles) { 
...
var L = zebra.layout, Rectangle = zebra.util.Rectangle, Layer = zebra.ui.Layer;
var Point = zebra.util.Point, Cursor = ui.Cursor, Dimension = zebra.util.Dimension
var Panel = zebra.ui.Panel, KeyListener = zebra.ui.KeyListener, MouseMotionListener = zebra.ui.MouseMotionListener;
var FocusListener = zebra.ui.FocusListener,  MouseListener = zebra.ui.MouseListener;
var ChildrenListener = zebra.ui.ChildrenListener, ComponentListener = zebra.ui.ComponentListener;
var ContainerListener = zebra.ui.ContainerListener, Composite = zebra.ui.Composite;
var KeyEvent = zebra.ui.KeyEvent, Cursorable = zebra.ui.Cursorable, rgb = zebra.util.rgb;

pkg.DgnComponent = new zebra.Interface();
...
pkg.Shaper = Class(MouseMotionListener, [
    ...
    function mouseDragged(e){
        if(this.target == e.source && this.state != null) {
            ...
            var nw = t.width  - dx * s.left + dx * s.right, nh = t.height - dy * s.top  + dy * s.bottom;;
...
pkg.ShaperPan = Class(Panel, Cursorable, Composite, FocusListener, KeyListener, [
    ...
    function setTarget(t){
        if (t != null && this.count() > 0) this.setTarget(null);

        var tt = (t == 0) ? this.get(0): t;
        var p  = tt.parent, tx = tt.x, ty = tt.y, tw = tt.width, th = t.height;
        var right = this.getRight(), top = this.getTop(), bottom = this.getBottom(), left = this.getLeft();
...
pkg.FormTreeModel = Class(zebra.data.TreeModelImpl, function($) {
...
//            if(this.selectedComp != null) this.repaint(this.brX, this.brY, this.brW, this.brH);
    ...
    $(function paintOnTop(g){
        if(this.selectedComp != null){
            ...
            if(this.selectedComp != this) this.borderView.paint(g, this.brX, this.brY, this.brW, this.brH, this);
                ...
                var right = this.getRight(), top = this.getTop(), bottom = this.getBottom(), left = this.getLeft();
                var ew = width - left - right, eh = height - top - bottom;;
    ...
    $(function moveComponent(){
        try{
            ...
            if(prevParent != null){
                ...
                var p = new Point(this.absX, this.absY);
                if (newParent != this) L.getRelLocation(this.absX, this.absY, newParent, this, p);
                ...
                this.selectedComp.setLocation(p.x, p.y);
                newParent.validate();
                this.calcMetric();
    ...
    $(function resizeComponent(){
        var p = {};
        L.getRelLocation(this.absX, this.absY, this.selectedComp.parent, this, p);
        var px = this.selectedComp.x, py = this.selectedComp.y, pw = this.selectedComp.width, ph = this.selectedComp.height;
        this.selectedComp.setLocation(p.x, p.y);
        this.selectedComp.setSize(this.absW, this.absH);
        this.selectedComp.invalidate();
        this.validate();
        this.calcMetric();
        if(px != p.x || py != p.y) this.support.perform(this, FormEditor.COMPONENT_MOVED, this.selectedComp);
...
pkg.Wizard = Class(Wizard, function($) {
    ...
    $(function() {
        ...
        this.customize = function(id,c){
            switch(id) {
                case Wizard.TFIELD: c.padding(2, 4, 2, 4);  break;
