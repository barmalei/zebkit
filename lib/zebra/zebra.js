            ...
            if (typeof m  === "function") {
                if (m.$clone$) {
                    for(var k in m.methods) {
                        var arity = parseInt(k, 10);
                        if (arity > 0) {
                            if (arity > 1) {
                                m = (function(m) {
                                    return function(v) { m.apply(obj, v); };
                                })(m);
                            }
                            break;
                        }
...
pkg.Class = make_template(null, function() {
    ...
    var df = arguments[arguments.length - 1], $parent = null,
    ...
    function $prototype() {
        ...
        this.doLayout = function(c){
            var rows = this.rows, cols = this.cols,
                ...
                right    = c.getRight(), top = c.getTop(), 
                bottom   = c.getBottom(), left = c.getLeft();
        ...
        this.setKids = function(kids) {
            if (Array.isArray(kids)) {
                for(var i=0; i<kids.length; i++) {
                    this.insert(i, kids.constraints, kids[i]);
                }
            }
            else {
                for(var k in kids) {
...
pkg.zCanvas = Class(pkg.Panel, [
    ...
    function $prototype() {
        ...
        this.recalcOffset = function() {
            ...
            this.offx = ba.left + measure(this.canvas, "padding-left") + window.pageXOffset;
            this.offy = ba.top  + measure(this.canvas, "padding-top")  + window.pageYOffset;

            //!!!!!!!!!!!! the code is not necessary any more since all modern browsers support
            ...
            // }
            //this.offx += measure(this.canvas, "padding-left");
            //this.offy += measure(this.canvas, "padding-top");
        };
    },
    ...
    if (zebra.isInBrowser) {
        ...
        if (e == null) {
            ...
            e.innerHTML = "<span id='zebra.fm.text'  style='display:inline;vertical-align:baseline;'>&nbsp;</span>" +
                          "<img  id='zebra.fm.image' style='display:inline;vertical-align:baseline;' src='data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABAQMAAAAl21bKAAAAA1BMVEUAAACnej3aAAAAAXRSTlMAQObYZgAAAApJREFUCNdjYAAAAAIAAeIhvDMAAAAASUVORK5CYII%3D' width='1' height='1'/>";
        ...
        if (zebra.isInBrowser && pkg.useWebClipboard) {
            ...
            $clipboard.oncopy = function(ee) {
                ...
                if (pkg.focusManager.focusOwner && pkg.focusManager.focusOwner.copy) {
                    $clipboard.value = pkg.focusManager.focusOwner.copy();
                    $clipboard.select();
                }
            }
...
pkg.TextRender = Class(pkg.Render, zebra.util.Position.PositionMetric, [
    function $prototype() {
        ...
        this.setText = function (s) { this.target.setText(s); };
...
pkg.Label = Class(pkg.ViewPan, [
    function $prototype() {
        this.getText = function(){ return this.view.getText(); };
...
pkg.Toolbar = Class(pkg.Panel, ChildrenListener, [
    ...
    function select(c,state){
        if(c != this.selected || (this.selected != null && state != this.isOver)){
            var prev = this.selected, addW = this.leftShift + this.rightShift, 
                addH = this.topShift + this.bottomShift;
            this.selected = c;
            this.isOver = state;
    ...
    function $clazz() {
        ...
        this.CompListModel = Class([
            function $prototype() {
                ...
                this.add = function(o){ 
                    var l = zebra.isString(o)? new pkg.CompList.Label(s) : o;
                    this.src.add(l); 
                };
...
pkg.TextField = Class(pkg.Label, pkg.KeyListener, pkg.MouseListener, pkg.MouseMotionListener,
    ...
    function $prototype() {
        ...
        this.remove = function (pos,size){
            ...
            if (pos >= 0 && (pos + size) <= position.metrics.getMaxOffset()) {
                if (this.isEditable && size < 10000) { 
                    ...
                    this.history[this.historyPos] = [-1, pos, this.getText().substring(pos, pos+size)]; 
        ...
        this.paintOnTop = function(g) {
            if (this.hint && this.hasFocus() === false && this.getText() == '') {
                this.hint.paint(g, this.getLeft(), this.height - this.getBottom() - this.hint.font.height, 
                                this.width, this.height, this);
            }
        };
    ...
    function (render){
        ...
        this.historyPos = -1;
        this.redoCounter = this.undoCounter = 0;
    ...
    function pageSize(){
        var height = this.height - this.getTop() - this.getBottom(),
            render = this.view, indent = render.getLineIndent(), 
            textHeight = render.font.height;
...
pkg.DefViews = Class([
    function $prototype() {
        this.getView = function (d, obj){
            this.defaultRender.target.setText(obj.value == null ? "<null>" : obj.value);
...
pkg.Tree = Class(ui.Panel, ui.MouseListener, ui.KeyListener,
    ...
    function $prototype() {
        ...
        this.getImageSize = function (i){
            return i.kids.length ? (this.getIM(i).isOpen ? this.viewSizes["open"]
...
pkg.DefEditors = Class([
    function getEditor(t, row, col, o){
        ...
        this.editor.setPaddings(t.cellInsetsTop + t.lineSize, t.cellInsetsLeft,
...
pkg.Grid = Class(ui.Panel, ui.MouseListener, ui.KeyListener, Position.PositionMetric,
        ...
        function $prototype() {
            ...
            this.paint = function(g){
                ...
                if(this.visibility.hasVisibleCells()){
                    ...
                    try{
                        g.save();
                        g.translate(this.sman.getSX(), this.sman.getSY());
            ...
            this.drawLines = function(hor, ver){
        ...
        function (data){
            ...
            this.sman = this.position = this.editors = this.stub = null;
        ...
        function setModel(d){
            if (d != this.model){
                this.clearSelect();
                if (Array.isArray(d)) model = new Matrix(d);
                if(this.model != null && this.model._) this.model._.remove(this);
                this.model = d;
                if (this.model != null && this.model._) this.model._.add(this);
...
pkg.GridStretchPan = Class(ui.Panel, zebra.layout.Layout, [
    function $prototype() {
        ...
        this.calcRowHeights = function(targetAreaW,targetAreaH,widths) { 
            var g = this.grid, rows = g.getGridRows(), sw = 0;
            for(var i = 0;i < rows; i++){
                var w = g.getRowHeight(i);
                if(w === 0) w = g.getRowPSHeight(i);
                sw += w;
            }
            var props = Array(rows);
            for(var i = 0;i < rows; i++){
                var w = g.getRowHeight(i);
                if(w === 0) w = g.getRowPSHeight(i);
                props[i] = w / sw;
        ...
        this.calcColWidths = function (targetAreaW,targetAreaH){
            var grid = this.grid, w = new Array(grid.getGridCols()),
        ...
        this.recalcPS = function (){
            ...
            if (isScr === true && p.height > 0 && p.vBar)
            {
...
pkg.InsetsCursorArea = Class(Cursorable, [
    function $prototype() {
        this.getCursorType = function (target,x,y){
            ...
            return (areaType >= 0) ? this.getAreaCursor(areaType) :  -1;
        };

        this.getAreaCursor = function(t){ return this.cursors[t]; };
    ...
    function (){
        ...
        this.setAreaCursor(L.LEFT, Cursor.W_RESIZE);
        this.setAreaCursor(L.RIGHT, Cursor.E_RESIZE);
        this.setAreaCursor(L.TOP, Cursor.N_RESIZE);
        this.setAreaCursor(L.BOTTOM, Cursor.S_RESIZE);
        this.setAreaCursor(L.TLEFT, Cursor.NW_RESIZE);
        this.setAreaCursor(L.TRIGHT, Cursor.NE_RESIZE);
        this.setAreaCursor(L.BLEFT, Cursor.SW_RESIZE);
        this.setAreaCursor(L.BRIGHT, Cursor.SE_RESIZE);
        this.setAreaCursor(L.CENTER, Cursor.MOVE);
    },

    function setAreaCursor(t,c){ this.cursors[t] = c; }
]);
...
pkg.ShaperPan = Class(ui.Panel, Cursorable, Composite, FocusListener, KeyListener, [
    ...
    function focusGained(e){
        this.shaperBr.color = this.colors[1];
        this.repaint();
    },
