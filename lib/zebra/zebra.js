    ...
    function $prototype() {
        this.paint =  function(g,x,y,w,h,d) {
            if (this.direction == L.VERTICAL) {
                ...
                g.lineTo(x + w, y + h - r);
                g.arc(x + r, y + h - r, r, 0, Math.PI);
                g.lineTo(x, y + r);
            }
            else {
                var r = h/2;    
                g.beginPath();
                g.arc(x + r, y + r, r, 0.5 * Math.PI, 1.5 * Math.PI);
                g.lineTo(x + w - r, y);
                g.arc(x + w - r, y + h - r, r, 1.5*Math.PI, 0.5*Math.PI);
                g.lineTo(x + r, y + h);
            }
            g.setColor(this.color);
            g.fill();
...
pkg.RadioView = Class(pkg.View, [
    ...
    function $prototype() {
        this.paint = function(g,x,y,w,h,d){
            ...
            g.arc(x + w/2, y + h/2 , w/3 , 0, 2* Math.PI, 1);
            g.fill();

            g.beginPath();
            g.fillStyle = this.color2;
            g.arc(x + w/2, y + h/2 , w/4 , 0, 2* Math.PI, 1);
            g.fill();
        };
    }
]);
                ...
                if (this.visibility.hasVisibleCells()) {
                    ...
                    try{
                        ...
                        if (this.leftCaption != null || this.topCaption != null) {
                            var cx = ts.x, cy = ts.y, ch = ts.height, 
                                dx = this.sman.getSX(), dy = this.sman.getSY();
...
pkg.Shaper = Class(MouseMotionListener, [
    function $prototype() {
        ...
        this.targetWasChanged = function(n,o){
            if(o != null){
...
pkg.ShaperPan = Class(ui.Panel, Cursorable, Composite, KeyListener, [
    function $prototype() {
        this.getCursorType = function (t, x ,y) { 
            return this.hasTarget ? this.shaperBr.getCursorType(t, x, y) : -1; 
        };
