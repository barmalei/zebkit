...
pkg.ListModelListener = new Interface();
pkg.ListModel = new Interface();
    
pkg.TextModelListener = new Interface();
pkg.TextModel = new Interface();

pkg.TreeModelListener = new Interface();
pkg.TreeModel = new Interface();

pkg.MatrixModel = new Interface();
pkg.MatrixModelListener = new Interface();
...
pkg.Text = Class(pkg.TextModel, function($) {
    ...
    $(function setText(text){
        ...
        if (old !== text) {
            if (old.length > 0) {
                ...
                this.lines.length = 0;
                this.lines = [ new Line("") ];
                this.perform(TML.REMOVED, 0, txtLen, 0, numLines);
            }
            
            this.lines = [];
            this.parse(0, text, this.lines);
            this.textLength = text.length;
            this.perform(TML.INSERTED, 0, this.textLength, 0, this.getLines());
        }
    });
    ...
    $(function write(s, offset){
        ...
        this.perform(TML.INSERTED, offset, slen, info[0], j);
    });
    ...
    $(function remove(offset,size){
        ...
        this.perform(TML.REMOVED, offset, size, i1[0], i2[0] - i1[0] + 1);
    });
    ...
    $(function perform(id,off,size,startLine,lines){
        if(this.support != null) {
            for(var i = 0;i < this.support.length; i ++ ) {
                this.support[i].textUpdated(this, id, off, size, startLine, lines);
            }
        }
    });

    $(function addTextModelListener(l){
        if(this.support == null) this.support = [];
        this.support.push(l);
    });

    $(function removeTextModelListener(l){ if(this.support != null) rm(this.support, l); });
    $(function getTextLength(){ return this.textLength; });
    $(function getExtraChar(i) { return this.lines[i].l; });
    $(function setExtraChar(i,ch){ this.lines[i].l = ch; });
...
pkg.SingleLineTxt = Class(pkg.TextModel, [
    ...
    function setText(text){
        ...
        if(this.buf == null || this.buf !== text) {
            if (this.buf != null && this.buf.length > 0) this.perform(TML.REMOVED, 0, this.buf.length);
            if (this.maxLen > 0 && text.length > this.maxLen) text = text.substring(0, this.maxLen); 
            this.buf = text;
            this.perform(TML.INSERTED, 0, text.length);
        }
    },
    ...
    function write(s,offset){
        ...
        if (l!=0) {
            ...
            if (l > 0) this.perform(TML.INSERTED, offset, l); 
        }
    },

    function remove(offset,size){
        this.buf = [ this.buf.substring(0, offset), this.buf.substring(offset + size)].join(''); 
        this.perform(TML.REMOVED, offset, size);
    },

    function perform(id,offset,size){
        if(this.support != null){
            for(var i = 0;i < this.support.length; i ++ ){
                this.support[i].textUpdated(this, id, offset, size, 0, 1);
            }
        }
    },
    ...
    function getExtraChar(i){ return this.extra; },
    function setExtraChar(i,ch){ this.extra = ch; }
]);
...
pkg.List = Class(pkg.ListModel, [
    ...
    function elementAt(i){
        if (i < 0 || i >= this.d.length) throw new IllegalArgumentException("" + i);
        return this.d[i];
    },
    ...
    function insertElementAt(o,i){
        if(i < 0 || i >= this.d.length) throw new IllegalArgumentException();
        this.d.splice(i, 0, o); 
...
pkg.TreeModelImpl = Class(pkg.TreeModel, function($) {
    ...
    $(function setRoot(r){
        if(this.root != null && r == null) throw new IllegalArgumentException();
        this.root = r;
    ...
    $(function addTreeModelListener(l){ this.addListener(l); });
    $(function removeTreeModelListener(l){ this.removeListener(l); });
...
pkg.Matrix = Class(pkg.MatrixModel,function($) {
    ...
    $(function removeRows(begrow,count){
        if(begrow < 0 || begrow + count > this.rows) throw new IllegalArgumentException();
    ...
    $(function removeCols(begcol,count){
        if(begcol < 0 || begcol + count > this.cols) throw new IllegalArgumentException();
