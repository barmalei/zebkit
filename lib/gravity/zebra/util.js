...
var Color = pkg.Color, Point = pkg.Point;
...
pkg.Bag = Class(function($) {
    ...
    $(function createObject(s, pos){            
        ...
        switch(ch)
        {
            ...
            case '&': {
                ...
                if(ch == '%') {
                    if (i1 < 0) {
                        desc.obj = pkg.Color[s.substring(pos + 1, desc.pos)];
                    }
                    else {
                        desc.obj = new pkg.Color(parseInt(s.substring(pos + 1, i1).trim()), 
                                                 parseInt(s.substring(i1 + 1, i2).trim()), 
                                                 parseInt(s.substring(i2 + 1, desc.pos).trim()));
                    }
                }
                else{
                    desc.obj = new pkg.Font(s.substring(pos + 1, i1).trim(), parseInt(s.substring(i1 + 1, i2).trim()), 
                                                                             parseInt(s.substring(i2 + 1, desc.pos).trim()));
                }
            } break;
            case '\"': {
            ...
            case '>' : { 
                var i = s.indexOf(',', pos + 1);
                desc.pos = seekStop(s, i + 1);
                desc.obj = new pkg.Dimension(parseInt(s.substring(pos + 1, i).trim()), 
    ...
    function resolveIdentifier(name) {
        ...
        if (pkg.Color[name]) return pkg.Color[name];
