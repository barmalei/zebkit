...
var Dimension = JAVA.awt.Dimension, Rectangle = JAVA.awt.Rectangle, L = zebra.layout, Position = zebra.util.Position, KE = pkg.KeyEvent; 
...
pkg.BaseList = Class(pkg.Panel, pkg.MouseMotionListener, pkg.MouseListener, pkg.KeyListener,
    ...
    $(function select(index){
        if(index >= this.model.elementsCount()){
            throw new IllegalArgumentException("index=" + index + ",max=" + this.model.elementsCount());
        }
...
pkg.List = Class(pkg.BaseList, function($) {
    ...
    $(function drawSelMarker(g,x,y,w,h){
        this.$super(this.drawSelMarker,g, x, y, this.width - this.getRight() - x, h);
        //this.$super(this.drawSelMarker,g, x, y, w, h);
    });

    $(function drawPosMarker(g,x,y,w,h){
        this.$super(this.drawPosMarker,g, x, y, this.width - this.getRight() - x, h);
        //this.$super(this.drawPosMarker,g, x, y, w, h);
    });
    ...
    $(function scrolled(psx,psy){
        this.firstVisible =  - 1;
        this.iVisibility();
        this.$super(this.scrolled,psx, psy);
    });
...
pkg.CompList = Class(pkg.BaseList, pkg.Composite, zebra.data.ListModel, [
    ...
    function setModel(m){
        if(m != this) throw new IllegalArgumentException("" + m);
        this.$super(this.setModel,m);
    },

    function setPosition(c){
        if(c != this.position){
            if (zebra.instanceOf(this.getLayout(), Position.PositionMetric)) c.setPositionMetric(this.getLayout());
            this.$super(this.setPosition,c);
        }
    },

    function setLayout(l){
         if(l != this.getLayout()){
             this.$super(this.setLayout,l);
    ...
    function focusGained(e){
        ...
        this.$super(this.focusGained,e);
    },

    function drawSelMarker(g,x,y,w,h){
        if (this.input == null || !L.isAncestorOf(this.input, pkg.focusManager.focusOwner)) this.$super(this.drawSelMarker,g, x, y, w, h); 
    },
...
pkg.Combo = Class(pkg.Panel, pkg.MouseListener, pkg.KeyListener, pkg.Composite, pkg.FocusListener, function($) {        
    var ContentComponent = Class(pkg.Panel, function($) {
        ...
        $(function paintOnTop(g){
            ...
            if(v != null){
                ...
                v.paint(g, this.getLeft(), this.getTop() + Math.floor((vpH - ps.height) / 2), vpW, vpH, this);
            }
        });
    ...
    var EditableContentPan = Class(ContentComponent, zebra.data.TextModelListener, pkg.FocusListener, [
        ...
        function setContentComponentListener(o){
            ...
            if(owner != o){
                if(o == null) this.textField.getModel().removeTextModelListener(this);
                this.$super(this.setContentComponentListener,o);
                if(o != null) this.textField.getModel().addTextModelListener(this);
            }
        },
    ...
    $(function kidAdded(index,s,c){
        this.$super(this.kidAdded,index, s, c);
    ...
    $(function kidRemoved(index,l){
        this.$super(this.kidRemoved,index, l);
        if(this.button == l){
    ...
    $(function paintOnTop(g){
        if(this.arrowGlyph != null && this.button != null){
            ...
            this.arrowGlyph.paint(g, b.x + Math.floor((b.width  - ps.width ) / 2), 
                                     b.y + Math.floor((b.height - ps.height) / 2), ps.width, ps.height, this);
        }
    });
